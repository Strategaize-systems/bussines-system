// =============================================================
// Meeting Invite Template (Basic) — DE
// =============================================================
// Simple invite email with individualized Jitsi meeting link.
// No .ics attachment — that comes in SLC-417.

type TemplateInput = {
  firstName: string;
  lastName: string;
  meetingTitle: string;
  meetingDate: string;   // formatted date string, e.g. "17. April 2026, 14:00 Uhr"
  meetingUrl: string;    // full Jitsi URL with JWT
  hostName: string;      // who is inviting
  agenda?: string;
};
...
export function meetingInviteSubject(input: TemplateInput): string {
  return `Einladung: ${input.meetingTitle}`;
}

export function meetingInviteText(input: TemplateInput): string {
...
export function meetingInviteHtml(input: TemplateInput): string {
