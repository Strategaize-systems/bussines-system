...
describe("generateSlug", () => {
  it("converts standard display name to kebab-case slug", () => {
    expect(generateSlug("Strategaize Transition BV")).toBe(
      "strategaize-transition-bv",
    );
  });

  it("strips ampersand and collapses spaces", () => {
    expect(generateSlug("AB & CD Beratung GmbH")).toBe("ab-cd-beratung-gmbh");
  });
  ...
  it("throws on input that has no alphanumeric chars after normalization", () => {
    expect(() => generateSlug("---")).toThrow(/alphanumeric/i);
    expect(() => generateSlug("&&&")).toThrow(/alphanumeric/i);
  });
...
describe("generateUniqueSlug", () => {
  ...
  it("treats reserved slug as collision, returns -2 suffix", () => {
    ...
    expect(generateUniqueSlug("Dashboard", new Set())).toBe("dashboard-2");
  });
  ...
  it("handles multi-collision chain across reserved + existing", () => {
    expect(
      generateUniqueSlug("Admin", new Set(["admin-2", "admin-3"])),
    ).toBe("admin-4");
  });
});
