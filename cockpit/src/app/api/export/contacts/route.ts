...
export async function GET(request: NextRequest) {
  ...
  let query = supabase
    .from("contacts")
    .select(
      "id, first_name, last_name, email, phone, position, company_id, relationship_type, role_in_process, trust_level, referral_capability, is_multiplier, multiplier_type, source, region, language, tags, created_at, updated_at, companies(name)"
    )
