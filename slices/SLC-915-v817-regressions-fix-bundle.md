...
- **Body-Route-Quelle = `email_messages` (Plan-QA-aufgelöst, korrigiert DEC-306-Wording `emails.id`):** Der Inbound-Viewer rendert `email.body_html` mit `email: InboxEmail`; `InboxEmail extends EmailMessage` (`imap-actions.ts:12`) und ALLE `getInboxEmail*` selektieren `.from("email_messages")` (`imap-actions.ts:39/69/96/...`). Die `.from("emails")`-Queries (`emails/actions.ts`) sind eine **separate** Entity (nicht der Inbound-Viewer). → Body-Route MUSS `email_messages` an `id` laden (`email_messages.body_html` existiert, RLS-scoped wie `getInboxEmails`). **OQ-A2 downgraded** auf Live-Confirm: RLS-Read + `body_html`-Spalte auf `email_messages` gegen Live-Schema bestätigen (nicht mehr „welche Tabelle").
...
- Expected behavior:
  ...
  2. **NEU** `GET /api/emails/[id]/body`: Same-Origin, lädt `body_html` per **User-Client (RLS-scoped)** aus **`email_messages` an `id`** (Plan-QA-aufgelöst — Viewer-Quelle, siehe Schema-Grounding; OQ-A2 nur noch Live-Confirm RLS+`body_html`-Spalte), `sanitizeEmailHtml(body,{blockRemoteImages:false})`, Response-Header route-scoped `Content-Security-Policy: default-src 'none'; img-src https: data: blob: cid:; style-src 'unsafe-inline'; font-src 'self'`. Globale CSP unberührt.
