...
- Consequence: MIG-028 setzt `proposals.tax_rate DEFAULT 21.00` und fuegt `CHECK (tax_rate IN (0.00, 9.00, 19.00, 21.00))` hinzu. Legacy-19%-Rows bleiben unangetastet sichtbar. Neue Angebote kriegen 21% als Default. Editor-Dropdown rendert nur "21% (Standard NL)", "9% (reduziert NL)", "0% (steuerfrei / Reverse-Charge)" — der 19%-Slot ist NICHT in der UI-Auswahl. Wenn eine alte Angebot-Version (mit 19%) per `createProposalVersion` kopiert wird, bekommt die Kopie initial das Original-19% (Snapshot), und der User kann im Editor explizit auf 21/9/0 wechseln. Migration ist idempotent (`DROP CONSTRAINT IF EXISTS` + `ADD CONSTRAINT`). KEINE Daten-Migration der 19%-Rows.
...
- Status: accepted
- Reason: Open Question 3 fragt nach Format-only vs. VIES-Real-Time-Lookup. VIES-Lookup (https://ec.europa.eu/taxation_customs/vies/) ist ein externer SOAP/REST-Service mit unklarer Verfuegbarkeit, Caching-Pflicht (24h-TTL ueblich), und neuer Code-Adapter mit Error-Handling. Risiko: VIES-Down blockiert Save-Flow. V5.7-Scope ist klein (~5-7h) — VIES-Integration koennte +3-5h kosten und einen externen Service-Failure-Pfad einfuehren. Format-only-Pattern-Validation ist trivial: NL-Format Regex fuer Strategaize (`^NL\d{9}B\d{2}$`), EU-Format-Regex fuer Empfaenger (`^[A-Z]{2}[A-Z0-9]{2,12}$` + Country-Code-Whitelist fuer EU-Mitglieder). User trippt sich nicht ueber Tippfehler an, aber kann theoretisch eine erfundene Nummer eingeben — das ist akzeptabel fuer Internal-Test-Mode und kann mit BL-XXX-VIES-Lookup spaeter gehaerten werden.
- Consequence: MIG-028 fuegt `branding_settings.vat_id TEXT NULL` und `companies.vat_id TEXT NULL` hinzu. Validation in `cockpit/src/lib/validation/vat-id.ts` (neu): `validateNlVatId(input)` und `validateEuVatId(input)` mit Regex + Country-Code-Whitelist (27 EU-Mitgliedstaaten 2026). Settings-Page `/settings/branding` ergaenzt das BTW-Eingabefeld nach dem Footer-Markdown-Block, mit inline Format-Error. Company-Stammdaten-Edit-Form ergaenzt das BTW-Feld nach `address_country`. Beide Felder NULLable — User kann sie spaeter nachtragen. KEIN VIES-Lookup in V5.7. Backlog-Item `BL-420 VIES-Lookup-Integration` wird angelegt fuer spaeter.
