...
## DEC-229 — V8.1 Teamlead-Tile-Sichtbarkeit ohne `/settings/team`-Page-Refactor: V7-Limited-Edit-Verhalten respektieren (V8.1 SLC-823)
- Status: accepted
- Reason: User-Discovery 2026-05-20 sagte "Tile sichtbar fuer Admin+Teamlead, klickbar nur fuer Admin (Read-Only fuer Teamlead)". Code-Audit zeigt: V7-Design ist Limited-Edit, NICHT reines Read-Only — Teamlead darf einladen (eigenes Team), Role-Change und Delete sind admin-only (`team-members-table.tsx`: `callerIsAdmin && !isSelf` Z.159, Z.204; `invite-dialog.tsx`: Admin-Option nur fuer Admin Z.151; Server-Actions: `inviteMember` admin+teamlead, `changeRole`+`deleteProfile` admin-only). Diskrepanz Discovery vs V7-Realitaet: "Read-Only" wird als "kein voller Admin-Edit" interpretiert. Optionen: (A) V7-Design respektieren — Tile-Permission auf ADMIN_TEAMLEAD, Page-Internals unveraendert (V7.1 DEC-196 deckt Limited-Edit ab), (B) `/settings/team` zu echtem Read-Only fuer Teamlead refaktorieren (InviteDialog disablen + BulkReassign disablen) — bricht V7-Design, neuer Backlog-Item noetig, (C) Tile nicht ergaenzen, Teamlead-Zugang nur via Sidebar — bestehende Inkonsistenz wuerde bleiben, BL-484 nicht geschlossen. **Gewaehlt:** (A). Wenn User echtes Read-Only will, muss das separat als V8.2+-Backlog-Item dokumentiert werden, weil es V7-Design-Aenderung ist.
- Consequence: `app/(app)/settings/page.tsx:187` `visibleFor: ADMIN_ONLY` → `ADMIN_TEAMLEAD`. Optional Description-Neutralisierung "Team-Mitglieder, Rollen-Zuweisung und Drilldown verwalten" (Sprache so, dass sie fuer Admin und Teamlead funktioniert). Keine Aenderung an `/settings/team/page.tsx`, `team-members-table.tsx`, `invite-dialog.tsx`, `bulk-reassign-dialog.tsx`. V7-Verhalten bleibt: Teamlead kann eigenes Team einladen, nicht admin/delete/role-change.
