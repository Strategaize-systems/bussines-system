...
- Current Focus: **V7 SLC-704 /backend done 2026-05-12** (RPT-401). Owner-Wiring vollstaendig implementiert: Audit-Spreadsheet `docs/AUDIT_SERVER_ACTIONS_V7.md` als Inventur-Deliverable, ~75 Mutate-Server-Actions mit `await assertNotReadOnlyContext()` als first line (Mutate-Lockdown DEC-189), ~30 INSERT-Punkte in 8 V7-Kerntabellen mit `owner_user_id = (await getProfile()).user_id`, 3 Cron-Endpoints (meeting-briefing/meeting-summary/call-processing) erben owner aus Source-Record, Workflow-Engine (`executor.loadEntity` + create_task + create_activity) reicht `entity.ownerUserId` durch. Vitest 737/737 PASS (+4 neue owner-pass-Tests), ESLint 142e/54w = V6.6-Baseline (keine neuen Errors), Build bewusst nicht ausgefuehrt (Coolify-Deploy macht es). COMPLIANCE.md V7-Owner-Wiring-Section ergaenzt. Schema-Diskrepanzen dokumentiert (emails-vs-email_messages, tasks-vs-activities Legacy-Splits) — V7-non-blocker. Naechster Schritt: /qa SLC-704 mit RLS-Matrix Re-Run + Live-Smoke 3 Rollen gegen Coolify nach User-Deploy.
- Current Phase: V7 Multi-User + Teamlead-Sprint — 4/7 Slices done (SLC-701+702+703+704), bereit fuer /qa SLC-704.

## Immediate Next Steps
1. **(naechster Schritt) /qa SLC-704** — verifiziert AC1..AC10. Live-Smoke 3 Rollen × Create-Smoke gegen Hetzner + Cross-URL-404-Smoke + Cron-Trigger-Smoke + RLS-Matrix 96 Tests Re-Run + Insert-Default-Tests via node:20-Docker-Container gegen Coolify-DB.
2. **(nach /qa SLC-704)** User-Coolify-Redeploy + Live-PASS-Verifikation, dann /frontend SLC-705 (Team-Aggregat-Cockpit /team).
...
4. **(SLC-705..707 Reihenfolge zwingend)** SLC-705 (Team-Aggregat /team) → SLC-706 (Drilldown /team/[user_id]/... Read-Only) → SLC-707 (Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix). Pro Slice: /backend|/frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa V7 nach SLC-707.
