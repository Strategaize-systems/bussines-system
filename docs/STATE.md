...
- Current Focus: **V7.1 SLC-712a DONE 2026-05-15** via RPT-419 (Backend) + RPT-420 (/qa Live-Smoke). PipelineView readOnly + viewAsUserId Reuse-Pattern etabliert. /qa Live-Smoke via Playwright-MCP gegen `business.strategaizetransition.com` als qa-teamlead PASS: AC4 (Drilldown rendert volle PipelineView read-only mit Title "Pipeline (Read-Only)" + 4 Toggles + KPIs + 10 Stages), AC2 (Neuer-Deal-Button unsichtbar), AC3 (Pipeline-Switcher schickt auf `/team/{id}/pipeline/{slug}`), AC7 (Funnel + Listen + Stage-Filter + Kanban-Toggles funktionieren), Regression-Check Self-Pipeline (`Pipeline`-Title, Neuer-Deal sichtbar, `/pipeline/{slug}`-URLs), Drag-Drop-no-effect via 3-Layer-Code-Defense + Vitest 4/4 (Live-Drag nicht moeglich da Test-Pipelines leer). Vitest **765/765 PASS** (+4 neue), TSC clean, Build clean, Lint 143e/54w. SLC-712a deployed via Commit `e6e4f0b` + User-Coolify-Redeploy.
- Current Phase: V7.1 **SLC-712a DONE (Backend + /qa)**. Naechster Schritt: /backend SLC-712b — Aufgaben + Mein-Tag-Drilldown-Vollausbau (3 MTs, ~2-4h). Etabliertes Pattern aus SLC-712a wiederverwenden. Parallel moeglich: /backend SLC-713 (Defense-in-Depth Polish, kein Code-Konflikt). Total V7.1-Aufwand verbleibend ~4-7h (SLC-712b + SLC-713 + Gesamt-/qa).

## Immediate Next Steps
1. **(Mainline) `/backend SLC-712b`** — Aufgaben + Mein-Tag-Drilldown-Vollausbau (3 MTs, ~2-4h). Etabliertes readOnly + viewAsUserId-Pattern aus SLC-712a wiederverwenden fuer `/team/[user_id]/aufgaben` und `/team/[user_id]/mein-tag`. Inkl. KI-Workspace-scope auf target_user_id.
