...
- Current Focus: **V6.6 SLICE-PLANNING DONE 2026-05-09** (RPT-368). 7 Slice-Specs SLC-661..667 verfasst mit Acceptance Criteria + Micro-Tasks + QA-Fokus + Risk-Mitigation. Reihenfolge zwingend (DEC-176): SLC-661 Foundation `<KIWorkspace>` Component (~3-4h, MUSS zuerst, Mock-getestet) → SLC-662 Mein Tag erster Caller + Performance-Migration (~3-4h) → SLC-663 Deals-Listen-Seite (~2-3h, parallelisierbar) → SLC-664 Deal-Detail Layout-Swap (~3-4h, 4 Sub-Block-Commits gegen R3) → SLC-665 Activity-Sheet + Win/Loss-Auto-Trigger einziger Backend-Slice (~3-4h, MIG-032 first) → SLC-666 KI-Analyse-Cockpit (~2h, dritter Caller) → SLC-667 KI-Inventur + Kalender-Polish + Sidebar-Reorder (~2-3h, letzter Slice). Pro Slice: backend|frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa nach SLC-667 → /final-check → /go-live → /deploy als REL-028. Folge-BLs: BL-451 SSE-Streaming-UI, BL-452 pg_trgm-Index, BL-453 /performance-Cleanup. Naechster Schritt: /backend SLC-661 (Foundation Component, ~3-4h, 8 MTs).
- Current Phase: V6.6 Pre-V7-Audit-Sprint — Slice-Planning DONE, Implementation aktiv (SLC-661 als naechster Slice).

## Immediate Next Steps
1. **(naechste Session) /backend SLC-661** — Foundation `<KIWorkspace>` Component (~3-4h, 8 MTs). Reusable Component fuer 3 Caller (Mein Tag, Deal-Detail, Cockpit), isoliert ohne Caller getestet (Mock-Reports + Mock-Bedrock). Files: cockpit/src/components/ki-workspace/{KIWorkspace.tsx, types.ts, AnswerPane.tsx, hooks/useReportRun.ts, hooks/useVoiceCapture.ts, reports/registry.ts, _mock.ts} + cockpit/src/lib/ki-workspace/cache.ts. Vitest +14 Tests. KEIN Live-Smoke noetig (Component noch nicht in Page).
