...
- Current Focus: **V7 SLC-706 /frontend Live-Smoke teilweise PASS, Pipeline-Fix wartet auf 3. Redeploy 2026-05-13**. Letzter Push `9ab4379` (Pipeline-Page Flat-Table). Iterationen: Initial-Push `062574f` → Schema-Drift-Fix-1 `527e3de` (pipelines.slug/tasks.owner_user_id/calendar_events.owner_user_id Bugs gefixt) → Render-Bug-Fix `9ab4379` (Pipeline-Gruppierung auf Flat-Table umgestellt weil seed-Deals pipeline_id=NULL haben). Live-Smoke gegen `527e3de` PASS: AC10 Happy-Path (Banner+KPIs+Top-Deals), Aufgaben-Drilldown (Banner+3 KPIs+Empty), AC5 visual Mutate-Block (0 Buttons), AC9 Cross-Team-Block (fake-uuid→404), Self-Block (Teamlead-Self→404), AC4 audit_log 4 view_as-Eintraege mit korrektem actor+target+view_as_target_user_id+context. **Pending:** AC6 Pipeline-Drilldown gegen `9ab4379` (3. Redeploy ausstehend). **AC8 Defense-in-Depth-Gap dokumentiert als ISSUE-066** — assertNotReadOnlyContext greift nur in Server-Component-Render-Chain (AsyncLocalStorage), nicht in separaten Server-Action-Requests; Direct-Action-Call kann UI-Block umgehen, aber RLS schuetzt Cross-Team. Vitest 748/748 PASS, TSC clean.
- Current Phase: V7 Multi-User + Teamlead-Sprint — 6/7 Slices code-side done (SLC-701..706), Live-Smoke teilweise PASS, 1 Pipeline-Render-Fix wartet auf User-Coolify-Redeploy → dann /frontend SLC-707.

## Immediate Next Steps
1. **(naechster Schritt) 3. User-Coolify-Redeploy auf Commit `9ab4379`** — Pipeline-Page-Flat-Table-Fix. Danach Pipeline-Drilldown-Smoke (Flat-Tabelle mit Deal/Pipeline/Stage/Wert/NaechsteAktion/Faellig fuer Test-Member 1 = 20 Deals).
2. **(nach Pipeline-Smoke PASS)** RPT-406 anlegen + Test-User-Cleanup (Random-Hex-Passwoerter via Admin-API) + Records-Sync.
3. **(nach SLC-706 /qa abgeschlossen)** /frontend SLC-707 — Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix + ggf. ISSUE-066-Mitigation (Defense-in-Depth via Pfad-Header oder Server-Action-Wrapper).
4. **(nach SLC-707)** Gesamt-/qa V7 → /final-check V7 → /go-live V7 → /deploy V7 als REL-029.
