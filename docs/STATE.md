...
- Current Focus: **V6.6 SLC-664 /qa MIXED 2026-05-10 — 2 Blocker, Hotfix vor SLC-665** (RPT-374). Live-Smoke gegen Production (HEAD d710153) zeigt 18/20 ACs PASS: Header (6 Elemente + Pencil-Drawer + Stage-Direkt-Wechsel + Inline-Edit + Mein-Tag-Switch), KI-Workspace komplett (5 Berichts-Buttons gegen Bedrock + 4-Sektion-Markdown + audit_log-Trail mit scopeHash a3477d2145626f5e + status=succeeded), 4 Removals (AIBriefingPanel + KnowledgeQueryPanel + Signale-Toolbar + Edit-Tab) DOM-verifiziert, Mobile-5+2-Action-Bar + 2/3+1/3 vertikales Stacking. **2 Blocker (ISSUE-059+060):** Meeting-Dropdown UND Mehr-Aktionen-Three-Dots crashen Deal-Detail-Page beim Click — Base UI Error #31 (invalid DropdownMenuContent-Children) + React #418 Hydration-Cascade. AC3+AC4 sind UNERREICHBAR, User-blocking. SLC-664 zurueck auf `blocked`. 5 SLC-664-Files mit Lint-`any` Carryover. 3 Low-Findings (F3 Bedrock-Latenz 12s leicht ueber 10s, F4 audit_log.changes NULL, F5 entity_id == actor_id). Naechster Schritt: Hotfix Action-Bar-Dropdowns + Coolify-Redeploy + /qa Re-Run AC3+AC4 + dann /backend SLC-665 (oder optional /backend SLC-663 parallelisierbar).
- Current Phase: V6.6 Pre-V7-Audit-Sprint — Implementation aktiv (SLC-661+662 done, SLC-664 blocked nach /qa, 2/7 Slices vollstaendig done).

## Immediate Next Steps
1. **(naechster Schritt) Hotfix Action-Bar-Dropdowns** in `cockpit/src/components/deals/deal-action-bar.tsx`: ISSUE-059 (Meeting-Dropdown — MeetingSheet aus DropdownMenuContent ausziehen, state-controlled Pattern wie StartMeetingModal) + ISSUE-060 (Mehr-Aktionen — md:hidden direkt auf DropdownMenuItem-Children + EnrollButton-Trigger durch state-controlled Pattern ersetzen). User-Coolify-Redeploy + /qa Re-Run AC3+AC4. Beide Blocker resolven dann ISSUE-059+060 + SLC-664 status zurueck auf `done`.
2. **(nach SLC-664 Hotfix)** /backend SLC-665 — Activity-Sheet + Win/Loss-Auto-Trigger (einziger V6.6-Slice mit MIG-032 + automation-Engine-Erweiterung). FEAT-664 wird erst nach SLC-665 done. Optional vorher: /backend SLC-663 Deals-Listen-Seite (parallelisierbar, eigenstaendig, kein KI-Workspace-Caller).
