...
- Current Focus: **V7.1 SLC-712a Backend DONE 2026-05-15** via RPT-419. PipelineView readOnly + viewAsUserId Reuse-Pattern etabliert. 4 MTs durch: PipelineView+KanbanBoard+KanbanColumn+KanbanCard mit `readOnly`-Branch (kein DnD, kein DealSheet, kein "Neuer Deal", kein Aktions-Dropdown). `/team/[user_id]/pipeline/page.tsx` rewrite + `/team/[user_id]/pipeline/[slug]/page.tsx` neu fuer Pipeline-Switcher. `getDealsForPipeline` mit optionalem `ownerUserId`-Filter. 4 neue RTL-Vitest gruen, Vitest **765/765 PASS**, TSC clean, Build clean, Lint 143e/54w (Baseline 142e/54w +1 Line-Shift-Error pre-existing useMemo). MT-2 als dokumentierter No-Op (kein localStorage-Persistence in PipelineView).
- Current Phase: V7.1 **SLC-712a Backend code-side DONE**. Naechster Schritt: /qa SLC-712a — Live-Smoke gegen `business.strategaizetransition.com` (Playwright-MCP, AC4 + AC7 + Drag-Drop-no-effect + Cursor-State + Pipeline-Switcher). User-Coolify-Redeploy ist Pflicht-Aktion am Slice-Ende vor Live-Smoke (per feedback_no_intermediate_coolify_switches.md). Total V7.1-Aufwand verbleibend ~5-8h (SLC-712a-QA + SLC-712b + SLC-713).

## Immediate Next Steps
1. **(Mainline) `/qa SLC-712a`** — Live-Smoke via Playwright-MCP gegen Production. AC4 (Drilldown-Pipeline volle Toggles), AC7 (Stage-Filter + Funnel-Toggle + Forecast), Drag-Drop ohne Effekt, "Neuer Deal" + Aktions-Dropdown unsichtbar, Pipeline-Switcher schickt auf `/team/{id}/pipeline/{slug}`. User-Coolify-Redeploy `90d5aad`-Folgesha als Pflicht-Vorab-Aktion (per feedback_no_intermediate_coolify_switches.md).
