...
- Current Focus: **V7 SLC-705 /frontend code-complete 2026-05-13** (RPT-403). Team-Aggregat-Cockpit `/team` implementiert via 4-Subagent-Orchestrierung: Aggregate-Query-Layer (3 Functions, 6 Vitest-Cases) + Perf-Smoke-Script + Server-Component-Page + 4 KPICards + sortierbare Mitglieder-Tabelle (7 Spalten, NULLs-last) + Empty-State + Drilldown-Stub + KI-Workspace mit 3 Team-Reports (Underperformance/Burnout/Stale-Deals) + Foundation-Type-Erweiterung (KIWorkspaceContext +"team-cockpit", Scope +teamId, makeCacheKey, _shared.ts authorizeTeamReport+invokeTeamReport) + Playwright-Test-Recipe. 10 NEW + 5 MOD Files. TSC clean (0 SLC-705-Errors), Next-Build PASS mit /team-Route, ESLint clean, Vitest 737/737. 5 Spec-Drifts dokumentiert und gefixt (deals.value vs total_gross, status=active vs open, activities.completed_at IS NULL vs status=open, (app) vs (authenticated), Teamlead nicht in eigener Tabelle). Naechster Schritt: User-Coolify-Redeploy + /qa SLC-705 (Perf-Smoke gegen Hetzner-DB, Browser-Smoke via MCP-Playwright fuer Teamlead+Member-Redirect, Live-Bedrock-Smoke).
- Current Phase: V7 Multi-User + Teamlead-Sprint — 5/7 Slices done (SLC-701+702+703+704+705), bereit fuer User-Deploy + /qa SLC-705.

## Immediate Next Steps
1. **(naechster Schritt) User-Coolify-Redeploy mit SLC-705-Commit** zu Image-Tag aus naechstem Push (10 NEW + 5 MOD Files inkl. neuer `/team`-Route + 3 Team-Reports + Foundation-Type-Erweiterung).
2. **(nach Redeploy) /qa SLC-705** — AC1..AC10 verifizieren: (a) Perf-Smoke via node:20+`aggregate-perf-smoke.ts` gegen Coolify-DB, p95<500ms-Check; (b) Browser-Smoke via MCP-Playwright (Teamlead sieht KPIs+Tabelle+KI-Workspace, Member redirected zu /mein-tag); (c) Live-Bedrock-Smoke "Wer hat Underperformance?"+audit_log-Verifikation; (d) `npm run test:rls` mit erweitertem Include picked die neuen Team-Aggregat-Tests auf.
...
4. **(nach SLC-705 /qa)** /frontend SLC-706 (Drilldown /team/[user_id]/... Read-Only-Context-Setter, view_as-Audit) → SLC-707 (Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix). Pro Slice: /backend|/frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa V7 nach SLC-707.
