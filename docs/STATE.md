...
- Current Focus: **V7 SLC-705 /qa done 2026-05-13** (RPT-404). Team-Aggregat-Cockpit `/team` live-verifiziert gegen Image-Tag `03e2aa0` mit MIXED-Result: 9/10 ACs voll PASS + 1 HIGH-Issue ISSUE-065 entdeckt. PASS: TSC+Build+Lint+Vitest 737/737, `npm run test:rls` 114/114 inkl. 6 SLC-705-Aggregate-Tests, Perf-p95 KPIs=98.4ms / Members=98.3ms (5× Headroom vs 500ms-Target), Browser-Smoke Teamlead+Member (Redirect korrekt), Drilldown-404 wie Spec, Live-Bedrock-Smoke mit Markdown-Antwort+audit_log-Entry (action=ki_workspace_report, actor_id=Teamlead, scopeHash=eccef87f24bf9af3, status=succeeded), Sort-Toggle DESC→ASC verifiziert. ISSUE-065 HIGH: `getTeamMembers` nutzt `.single()` ohne explicit `auth.uid()`-Lookup → Self-Filter inaktiv → Tabelle 6 Rows statt 5 + Bedrock-Context enthaelt Teamlead-als-Member. Funktional kein Daten-Leak. Empfohlen: Hotfix-Commit vor SLC-706 (~30min). Test-User-Cleanup via Random-Hex-Passwoerter PUT.
- Current Phase: V7 Multi-User + Teamlead-Sprint — 5/7 Slices done (SLC-701+702+703+704+705), bereit fuer ISSUE-065-Hotfix + /frontend SLC-706.

## Immediate Next Steps
1. **(naechster Schritt) ISSUE-065 Hotfix** — `getTeamMembers` Self-Filter aktivieren via `auth.getUser()` + `.eq("id", user.id)` Pre-Lookup (~30min). Plus `aggregate-queries.test.ts` umstellen auf direkten `getTeamMembers()`-Aufruf statt SQL-Replication. Commit + User-Coolify-Redeploy + Quick-Smoke (Tabelle 5 Rows + Bedrock-Context-Member-Count). Vor SLC-706 abschliessen weil Drilldown-Routes pro Member-Row gebaut werden.
2. **(nach ISSUE-065)** /frontend SLC-706 (Drilldown /team/[user_id]/... Read-Only-Context-Setter, view_as-Audit) → SLC-707 (Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix). Pro Slice: /backend|/frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa V7 nach SLC-707.
