...
- Current Focus: **SLC-757 /slice-planning DONE 2026-05-18** — Spec-File `slices/SLC-757-issue077-nl-rule-builder-polish.md` mit 6 MTs angelegt (MT-1 sonner+next-themes-Deps + ui/sonner.tsx 1:1-Port aus Onboarding, MT-2 Toaster-Mount in RootLayout, MT-3 handleApply Toast + handleNewRule + Banner-Removal nach Variante A, MT-4 Vitest RTL, MT-5 Build+Lint+Records, MT-6 /qa+Live-Smoke). 10 ACs, 5 Risks dokumentiert. Pattern-Reuse-Search bestaetigt: Onboarding-Plattform `src/components/ui/sonner.tsx` + `src/app/layout.tsx` als kanonische Quelle. User-Entscheidungen 2026-05-18: Reset-Strategie Variante A (Banner faellt weg), next-themes 1:1 portieren. Backlog BL-477 (V7.5, open) + slices/INDEX.md SLC-757 planned. **Naechster Schritt: /frontend SLC-757 MT-1..MT-5** (Code-Schritt ~30-45 Min), danach /qa MT-6 Live-Smoke ~30 Min.
- Current Phase: V7.5 SLC-757 /slice-planning done. FEAT-751 Sculptor Code 6/6 Slices done, SLC-757 Polish-Slice planned. Code-Schritt steht aus.

## Immediate Next Steps
1. **(Mainline)** /frontend SLC-757 MT-1..MT-5 — Sonner-Install + Toaster-Mount + handleApply-Polish + Vitest + Records-Sync. Spec `slices/SLC-757-issue077-nl-rule-builder-polish.md`. ~30-45 Min Code.
2. **(direkt anschliessend)** /qa SLC-757 MT-6 — Live-Smoke gegen Coolify (Toast + Card-Reset + SQL-Verifikation + Cleanup) ~30 Min.
3. **(V7.5-Sequenz nach SLC-757)** Gesamt-/qa V7.5 → /final-check → /go-live → /deploy als REL-032.
...
9. **(V6.2-Dispatcher-Beobachtung aus SLC-754 Live-Smoke, nicht-blockierend)** V6.2-Dispatcher liest `automation_rules` ohne Owner-Scope-Filter (alle aktiven Rules mit matching trigger_event ueber alle User hinweg). NL-Rule ohne Conditions wuerde auf jeden Stage-Change feuern. Assignee=`trigger_user` resolved auf Deal-Owner statt Triggering-User. Nicht kritisch, aber dokumentiert fuer V8+ Multi-User-Scoping-Discussion.
