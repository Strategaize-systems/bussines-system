...
- Current Focus: **V6.6 SLC-665 AC12 Read-API LIVE-PASS 2026-05-11** — ISSUE-061 resolved via Commit `8379833` (publicPaths um `/api/winloss` ergaenzt). Live-curl 4/4 Faelle PASS gegen Container-Image `8379833`: ohne-Auth -> 401, falsche-Auth -> 401, gueltige-Auth+kein-Run -> 404, gueltige-Auth+Run -> 200 mit JSON aller 7 Felder. Static (Vitest 546/546, Build) + DB-Schema (MIG-032 + System-Rule) + Code-Wiring + Read-API alle PASS. UI-Smokes Blocks A/B/D (Activity-Sheet + 3-fach Stage-Toggling + Manueller Re-Run) erfordern User-Browser-Login. SLC-665 bleibt `in_progress` bis User-Browser-Smoke. Naechster Schritt: User-Browser-Smoke AC4..AC11+AC13.
...
1. **(naechster Schritt) User-Browser-Smoke SLC-665** — AC4..AC11+AC13 (Activity-Sheet 2x: Meeting-Activity mit Bedrock-Summary + Notiz ohne Summary; Auto-Trigger 3-fach Stage-Toggling won→lost→won mit audit_log-Pruefung auf genau 2x `auto_winloss_triggered` + 1x `auto_winloss_skipped_recent_run`; Manueller Re-Run-Button + `Erneut analysieren`-Bypass). Nach PASS: SLC-665 done, BL-448+BL-450 done.
