...
- Current Focus: **V7 SLC-703 Verwaltungs-UI /qa Live-Smoke 5/6 PASS 2026-05-12** (RPT-398 backend + RPT-399 qa-code + RPT-400 qa-live). Smokes 1+4+5+6 voll PASS, Smoke 3 (Mail-Empfang) blockiert von ISSUE-062 (Coolify-ENV `GOTRUE_SMTP_PASS=unused`-Drift). Hotfix `e616ffe` deployed: lax-UUID-Format (Seed-Pattern `00000000-...-081` IDs werden jetzt akzeptiert, vorher Zod-strict v4-Reject). Hard-Lock greift exakt: "Profil hat noch 174 aktive Records (companies: 10, contacts: 40, deals: 20, activities: 100, meetings: 1, proposals: 1, email_messages: 1, calls: 1). Vorher Bulk-Reassign noetig." Role-Change DB+audit_log perfekt. Teamlead-Cross-Team-Block UI+Server-Side verifiziert. 2 neue Findings: ISSUE-062 SMTP-Pass-Drift (HIGH, infra), ISSUE-063 Team-Dropdown UUID-Display (MEDIUM, UX-Polish). Naechster Schritt: /backend SLC-704 (Owner-Wiring ~80 Server Actions — V7-groesster Slice, eigene Session empfohlen).
- Current Phase: V7 Multi-User + Teamlead-Sprint — 3/7 Slices done (SLC-701+702+703), bereit fuer SLC-704 (Owner-Wiring).

## Immediate Next Steps
1. **(naechster Schritt) /backend SLC-704** — Owner-Wiring fuer ~80 bestehende Server Actions (Insert setzt owner_user_id, Listing/Detail-Queries via RLS implizit gefiltert). V7-groesster Slice, eigene Session empfohlen.
2. **(optional Parallel) User-Manual-Fix ISSUE-062 in Coolify-UI** — `GOTRUE_SMTP_PASS=aithatworks-01!` setzen + GoTrue-Container Redeploy. Macht Auth-Mails wieder lebendig (Invite/Password-Reset/Email-Change). Blockiert sonst Live-Verifikation von SLC-703 AC3 (Invite-Mail).
3. **(SLC-704..707 Reihenfolge zwingend)** SLC-704 (Owner-Wiring) → SLC-705 (Team-Aggregat /team) → SLC-706 (Drilldown /team/[user_id]/... Read-Only) → SLC-707 (Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix). Pro Slice: /backend|/frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa V7 nach SLC-707.
