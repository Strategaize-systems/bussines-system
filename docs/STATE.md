...
- Current Focus: **V5.7 SLC-571 7/9 MTs done 2026-05-04 (MT-5+MT-6+MT-7).** MT-5+MT-6 QA Live-PASS (RPT-294). **MT-7 Code-complete:** `validateReverseCharge` Pure-Function mit 4 Reject-Pfaden (tax_rate≠0 / branding.vat_id-empty / company.vat_id-empty / country-non-EU-oder-NL), 14/14 Vitest gruen. Wired in `updateProposal` Server-Action mit Resolved-State-Computation (post-Patch tax_rate + reverse_charge gegen branding+company-Snapshot fetched) BEVOR DB-UPDATE — verhindert Postgres-CHECK-Errors mit unbrauchbaren Fehlermeldungen + enforced Server-side-only Eligibility-Pfade (vat_id + country). Zusaetzlicher logAudit-Call mit `action='reverse_charge_toggled'` nur bei tatsaechlicher RC-Status-Aenderung (Spam-Schutz). AuditAction-Union erweitert. TSC + 222/222 Vitest + Build + Lint clean. Naechste = User-Coolify-Redeploy + /qa MT-7 (Reject-Pfad-Smoke via DevTools-Curl/Editor-Bypass) oder /backend MT-8 (PDF-Renderer reverse-charge-block.ts) parallel.
- Current Phase: V5.7 — SLC-571 in_progress (7/9 MTs done), naechste = User-Coolify-Redeploy + /qa MT-7 oder /backend MT-8

## Immediate Next Steps
1. **User: Coolify-Redeploy Business System** mit aktuellem Code-Stand (commit folgt) — MT-7-Server-Side-Validation + Audit-Insert sind nur in Production wirksam.
2. **/qa SLC-571 MT-7** — Reject-Pfad-Smoke: per DevTools-Network-Edit oder direktem Server-Action-Aufruf eine ungueltige Patch-Kombination senden (RC=true + tax_rate=21, RC=true ohne branding.vat_id, RC=true mit NL-company) und die deutschen Fehlermeldungen verifizieren. Audit-Log-Smoke: `/audit-log` zeigt `reverse_charge_toggled` Eintraege bei Toggle-ON/OFF mit korrektem changes-JSON.
3. **/backend SLC-571 MT-8** — PDF-Renderer reverse-charge-block.ts (NEU) + proposal-renderer.ts MODIFY (Block direkt unter Tax-Row + Footer-Strategaize-vat_id) + 4 Snapshot-Test-Cases (regression-frei zu V5.6 + 3 RC-Varianten). Fuer NL-VAT rechtlich erforderlich.
4. **MT-9** (COMPLIANCE.md + Cockpit-Records-Final) als finale Vorbereitung fuer /go-live.
...
## Active Scope
**V5.7 — NL+DE-Compliance + Polish (in_progress, Slice-Planning + 7/9 MTs done 2026-05-04 + MT-5/MT-6 QA Live-PASS):**
