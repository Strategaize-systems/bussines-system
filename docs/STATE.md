...
- Current Focus: **V5.7 SLC-571 6/9 MTs done + QA Live-PASS 2026-05-04 (MT-5+MT-6).** MT-5 useReverseChargeEligibility-Hook + countryNameToCode-Mapper (24/24 Vitest). MT-6 TaxRateDropdown + ReverseChargeSection + Editor-Wiring + Datenflusserweiterung (zod-Whitelist {0,7,9,19,21}, Proposal.reverse_charge, ProposalEditPayload um vat_id/business_country/address_country, AUDIT-Fields, getProposalForEdit-Select, createProposal-Country-Default NL=21/DE=19). Code-side TSC + 208/208 Vitest + Build + Lint clean. **/qa Live-PASS via RPT-294** — 7 Browser-Smoke-Permutationen alle gruen gegen 5015b26 Live: Legacy-Render, Save+Reload-Persistence, Eligibility-Setup (Branding+Company+Deal), New-Proposal-Default-21%, Toggle-ON/OFF mit Lock auf 0%, Restore-Pattern, NL-Empfaenger-Disabled-Tooltip. Findings: 1 Low (pre-existing Workspace-Dual-Render-IDs aus V5.5), 1 Open (Stale-RC-State bei Country-Wechsel — MT-7-Scope). Naechste = /backend SLC-571 MT-7 (saveProposal Server-Action-Validation + Audit-Insert) oder MT-8 (PDF-Renderer reverse-charge-block.ts).
- Current Phase: V5.7 — SLC-571 in_progress (6/9 MTs done + MT-5/MT-6 QA-PASS), naechste = /backend MT-7 saveProposal-Validation oder MT-8 PDF-Renderer

## Immediate Next Steps
1. **/backend SLC-571 MT-7** — saveProposal Server-Action-Validation: 4 Reject-Pfade (reverse_charge=true ohne tax_rate=0 / ohne branding.vatId / ohne company.vat_id / mit non-EU-country oder NL) + Audit-Insert bei Status-Aenderung (`action='reverse_charge_toggled'`, `meta={"to": true|false, "tax_rate_before": X, "tax_rate_after": Y}`). Vitest gegen Coolify-DB mit SAVEPOINT-Pattern.
2. **/backend SLC-571 MT-8** — PDF-Renderer reverse-charge-block.ts (NEU) + proposal-renderer.ts MODIFY (Block direkt unter Tax-Row + Footer-Strategaize-vat_id) + 4 Snapshot-Test-Cases (regression-frei zu V5.6 + 3 RC-Varianten). Fuer NL-VAT rechtlich erforderlich.
3. **MT-9** (COMPLIANCE.md + Cockpit-Records-Final) als finale Vorbereitung fuer /go-live.
