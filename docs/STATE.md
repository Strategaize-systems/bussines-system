...
- Current Focus: **V7 ISSUE-065 Hotfix done 2026-05-13** (code-side). `cockpit/src/lib/team/aggregate-queries.ts:185-209` umgebaut auf zweistufigen Lookup (`auth.getUser()` + `.eq("id", user.id).single()`). Plus neuer Unit-Test `cockpit/src/lib/team/aggregate-queries.test.ts` mit 8 vi.mock-Tests, der `getTeamMembers()` direkt aufruft (faengt zukuenftige `.single()-ohne-.eq()` Bug-Klasse). Vitest 745/745 PASS (737 baseline + 8 neu). TSC clean fuer SLC-705-Files (9 pre-existing unrelated Errors = baseline). Warten auf User-Coolify-Redeploy + Quick-Smoke (Tabelle 5 statt 6 Rows). Davor: V7 SLC-705 /qa 9/10 ACs PASS via RPT-404, Team-Aggregat-Cockpit `/team` live-verifiziert gegen Image-Tag `03e2aa0`.
- Current Phase: V7 Multi-User + Teamlead-Sprint — 5/7 Slices done (SLC-701..705), ISSUE-065 Hotfix code-side fertig, bereit fuer User-Coolify-Redeploy → Quick-Smoke → /frontend SLC-706.

## Immediate Next Steps
1. **(naechster Schritt) User-Coolify-Redeploy ISSUE-065-Hotfix** — Coolify-UI → cockpit-Resource → Force Rebuild auf neuestem main-Commit. Nach Deploy: Quick-Smoke (Teamlead-Login → `/team` → Tabelle muss 5 Rows zeigen, NICHT 6; KI-Workspace "Underperformance"-Klick → Bedrock-Antwort darf Teamlead nicht als Outlier identifizieren). Bei PASS: ISSUE-065 vollstaendig resolved.
2. **(nach Hotfix-Smoke)** /frontend SLC-706 (Drilldown /team/[user_id]/... Read-Only-Context-Setter, view_as-Audit) → SLC-707 (Polish + Bulk-Reassign-UI + Mobile-Hamburger + VERWALTUNG-Split + ISSUE-063-Fix). Pro Slice: /backend|/frontend → /qa → User-Coolify-Deploy → Live-Smoke. Gesamt-/qa V7 nach SLC-707.
