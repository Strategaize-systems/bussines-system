...
- Current Focus: **V6.6 SLC-667 code-side DONE 2026-05-11** — letzter V6.6-Hygiene-Slice. SparklesCard-Removal NO-OP (bereits weg). AI-Bereitschaft-Rename via zentrale `KI_READINESS_LABEL`-Map (Schema bleibt `ai_readiness`). Pipeline-NL-Suche durch `<TypeAheadSearch>` (SLC-663-Reuse) ersetzt + pipeline-search-bar.tsx gelöscht. Sidebar-Reorder ANALYSE-first (Dashboard) → OPERATIV → ARBEITSBEREICHE → VERWALTUNG (unverändert). Kalender Default-Range 06:00-21:00 (von 07:00-20:00) via `DEFAULT_HOUR_RANGE`-Konstante. Working-Hours Settings-Page `/settings/working-hours` mit 2 TimePickern + Server-Actions (validate + upsert auf user_settings, MIG-032 CHECK-Constraint). Kalender-Toggle "Voller Tag / Nur Arbeitstag" mit localStorage-Persistenz, Pure-Function `computeHourRange(mode, workingHours)` für Range-Berechnung. Vitest +21 (650/650 PASS). Build clean. Lint: 0 errors auf SLC-667-Dateien (pipeline-view.tsx Errors um 4 reduziert, Rest pre-existing). V6.6: alle 7/7 Slices code-side done (SLC-661..667), alle FEAT-661..666 done. Naechster Schritt: User-Coolify-Deploy + /qa SLC-667 + Gesamt-/qa V6.6.
- Current Phase: V6.6 Pre-V7-Audit-Sprint — Implementation komplett (7/7 Slices code-side done).

## Immediate Next Steps
1. **(naechster Schritt) User-Coolify-Deploy SLC-667** + /qa SLC-667 mit Live-Smoke aller 21 ACs (SparklesCard-Removal, AI-Bereitschaft-Label, Pipeline-Type-Ahead, Sidebar-Reorder, Kalender-Range 06-21, Working-Hours-Settings-Save, Toggle).
2. **(danach) Gesamt-/qa V6.6** → /final-check → /go-live → /deploy als REL-028.
...
**Active:**
- **V6.6 Pre-V7-Audit-Sprint** (Requirements DONE 2026-05-09 RPT-366, Architecture DONE 2026-05-09 RPT-367, Slice-Planning DONE 2026-05-10 RPT-368, **SLC-665 vollstaendig DONE 2026-05-11 RPT-379**): Konsolidiertes KI-Workspace-Hybrid-Pattern auf Mein Tag + Deal-Detail + Dashboard. Pipeline-Progress als Haupt-KPI. /performance-Seite faellt komplett. 6 Features definiert (FEAT-661..666), 7 Slices SLC-661..667, 16 DECs (DEC-165..180), MIG-032 applied. 4/7 Slices done: SLC-661 (Foundation), SLC-662 (Mein Tag + Performance-Migration), SLC-664 (Deal-Detail Layout-Swap), SLC-665 (Activity-Sheet + Win/Loss-Auto-Trigger inkl. ISSUE-061 Middleware-Fix). FEAT-664 done. FEAT-666 in_progress (Auto-Trigger live, Sparkles+Sidebar offen in SLC-667). Vitest 546/546 PASS. Naechster Schritt: /backend SLC-663 (Deals-Listen-Seite).
