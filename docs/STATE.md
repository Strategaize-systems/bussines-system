...
- Current Focus: **SLC-753 DONE 2026-05-17** (RPT-450 /qa Live-Smoke MT-5 PASS, alle 10 AC verifiziert gegen Production-Coolify auf `main` HEAD `bafe9ca`). 2 echte Production-Bedrock-Sculpt-Anfragen via Playwright-MCP, Member-DOM-Absence-Assertion PASS, audit_log cost=0.009312 persistiert. **Hotfix `bafe9ca` aus QA-Discovery**: ISSUE-076 PRICING-Coverage-Gap zwischen Coolify-ENV `LLM_MODEL=eu.anthropic.claude-sonnet-4-6` (Kurz-Form) und Lang-Form-only PRICING-Table — Auto-Fix per QA Rule 1 mit 2 Kurz-Form-Aliasen + 2 Vitest. Vitest 901/901 PASS, RLS-Suite unveraendert. Container `app-...082026020167` Up healthy. **V7.5-Progress: 3/6 done** (SLC-751 + SLC-752 + SLC-753); SLC-754..756 + Gesamt-/qa + /final-check + /go-live + /deploy als REL-032 offen. **Naechster Schritt: /slice-planning + /backend+/frontend SLC-754** Trockenlauf+Apply-Confirm.
- Current Phase: V7.5 SLC-753 done. SLC-754 next.

## Immediate Next Steps
1. **(Mainline)** /slice-planning + /backend+/frontend SLC-754 Trockenlauf-Karte + Apply-Confirmation-Modal. Spec liegt bereits in `slices/SLC-754-preview-apply-confirm.md`. Schaetzung ~3-5h.
2. **(Optional User-Action, nicht zeitkritisch)** `BEDROCK_REGION=eu-central-1` in Coolify-ENV explizit setzen als Discipline-Massnahme (Code-Pfad ist via AWS_REGION-Fallback abgesichert, daher nicht-blockierend). RPT-446 F-3. Plus: `LLM_MODEL` auf Lang-Form `eu.anthropic.claude-sonnet-4-6-20250514-v1:0` korrigieren als Best-Practice (Code-Pfad via Kurz-Form-Alias abgesichert, ISSUE-076 resolved).
