...
### ISSUE-089 — GoTrue signInWithPassword broken fuer alle bestehenden User (nur Signup-Pfad funktioniert)
- Status: open
- Severity: High
- Area: Auth / GoTrue / Self-Hosted
- Summary: BS V8.10 Gesamt-/qa Live-Smoke 2026-06-03 hat festgestellt, dass `/auth/v1/token?grant_type=password` fuer ALLE bestehenden User invalid_credentials returnt — auch nach SQL-Direkt-crypt()-Password-Reset (verified: `SELECT crypt('pwd', encrypted_password) = encrypted_password` returnt TRUE auf DB-Level). Founder `richard@bellaerts.de` letzter Login 2026-05-21 (vor ~2 Wochen). qa-Test-User letzter Login 2026-06-02 09:23 UTC (gestern, vor Coolify-Redeploy heute). Nach Coolify-Redeploy heute 06:56 UTC schlaegt JEDER signInWithPassword fehl. **Aber: `/auth/v1/signup` funktioniert** — neu angelegte User koennen sich sofort einloggen. Auch GoTrue-Container-Restart hilft nicht. Bcrypt-Hash-Format `$2a$10$` identisch zu funktionierenden Signup-User-Hashes.
...
- Workaround: Brand-neue Signup-User einloggen funktioniert. Bestehende User-Anlegung via Admin-API funktioniert auch (User wird angelegt mit `aud=""`, Hash ist OK), aber `signInWithPassword` schlaegt fehl.
- Discovery: 2026-06-03 BS V8.10 Gesamt-/qa Live-Smoke (RPT-569). GoTrue-Image `supabase/gotrue:v2.160.0`.
- Next Action: (a) GoTrue v2.160 Changelog auf signInWithPassword Validierungs-Aenderung pruefen. (b) Pruefen ob `password_required_characters` ENV o.ae. eine neue Anforderung an Password-Hash-Format einfuehrt. (c) qa-admin Password via signup-Pfad re-create (DELETE qa-admin + signup-form mit qa-admin@strategaize.test + identisches Password). (d) Cross-Repo-Check OP + IS + ImSch — gleicher Bug? (e) Pre-Customer-Live Blocker — muss vor V8.13 oder V9 gefixt sein.

### ISSUE-088 — Storage-API Self-Access INSERT RLS-Denial (Storage-Service vs. authenticated-Role Context-Bridge)
- Status: open
- Severity: High
- Area: Storage / RLS / Self-Hosted GoTrue
- Summary: BS V8.10 SLC-893 MT-6 + Gesamt-/qa Live-Smoke 2026-06-03 hat festgestellt: authenticated User koennen Storage-Objekte im `documents`-Bucket via Storage-API LESEN (SELECT-Policy funktioniert ✅), aber NICHT INSERIEREN (INSERT-Policy denies "new row violates row-level security policy" trotz korrekter Pfad-Struktur). Bestaetigt per echtem GoTrue-issued JWT in Cookie-Session (RPT-569): SELECT `/documents/<USER_A>/.../probe.txt` → HTTP 200 PASS; INSERT `/documents/<USER_A>/.../self-insert.txt` → HTTP 400 "violates RLS" FAIL. Storage-Service-Log zeigt `owner: "<user-uuid>"` + `role: "authenticated"` (JWT korrekt geparsed). Cross-Tenant-Defense (USER_B kann USER_A-Pfade nicht lesen/schreiben) funktioniert korrekt — SEC-008 ist gefixt. Aber Self-Access INSERT ist betroffen — selbes Verhalten in `proposal-pdfs`-Bucket (8 alte Files vom 2026-05-04, seither keine neuen Uploads).
- Impact: Production-Document-Uploads via `supabase.storage.from(bucket).upload(...)` (Browser-Cookie-Session oder signed-JWT) scheitern fuer alle user-scoped Buckets. Founder kann via UI keine Documents anlegen. Internal-Test-Mode aktuell nicht direkt blockierend (kein Customer affected). Bei Customer-Go-Live blockierend.
- Workaround: Server-Actions koennten temporaer `createAdminClient` statt `createClient` nutzen (service_role BYPASSRLS) — hebt aber RLS-Defense fuer Insert auf. Besser: Self-Access-Bug verstehen + fixen.
- Discovery: SLC-893 MT-6 Live-Smoke 2026-06-03 (RPT-568) + Gesamt-/qa Live-Smoke 2026-06-03 (RPT-569, mit echtem GoTrue-JWT bestaetigt SELECT-PASS + INSERT-FAIL).
- Next Action: (a) Storage-Service v1.11.13-Source pruefen: warum `auth.uid()` in INSERT-WITH-CHECK-Eval NULL returnt aber in SELECT-USING-Eval korrekt? (b) Cross-Repo-Check OP + IS + ImSch — sind dort Storage-Uploads auch broken? (c) Storage-Service Config-Drift seit V8.7-A Coolify-Redeploy untersuchen. (d) Pre-Customer-Live Blocker — muss vor V8.13 oder V9 gefixt sein. Verwandt zu [[ISSUE-089]] (signInWithPassword broken) — beides Coolify-Redeploy-side-effects-Kandidaten.
