...
- Next Action: TEIL-BEHOBEN (Ansatz a, DEC-307). MT-4 CODE-DONE 2026-07-08 (MIG-055, RPT-679): UPDATE-`WITH CHECK` der 9 Tabellen zurück auf OR + changed-FK-only BEFORE-UPDATE-Trigger (SECURITY INVOKER, service_role-aware); INSERT bleibt AND. OQ-A1 live in-tx-Probe gegen Coolify-DB 8/8 PASS (mixed-owner-Status-Update entsperrt, Cross-Tenant-FK-Injection UPDATE+INSERT weiter BLOCK). OFFEN für Status-Resolution: (1) MT-5 — die 4 Call-Sites (`followup-actions.ts:126`, `fit-assessment/signal-actions.ts:73`, `emails/imap-actions.ts:135`, `meetings/actions.ts:228`) auf Fehlerprüfung + Kompensation umstellen; (2) Live-Apply MIG-055 im /deploy (aktuell nur in-tx-verifiziert, nicht persistent). Fix-Slice: BL-520 / SLC-915.
