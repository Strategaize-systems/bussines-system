...
- Next Action: `connect-src` um `wss://sip.strategaizetransition.com` (bzw. wss-Variante von `NEXT_PUBLIC_SIP_DOMAIN`) erweitern + `Permissions-Policy` von `microphone=()` auf eine self-Allowlist (`microphone=(self)`, ggf. `camera=(self)`) für die Telefonie/Meeting-Flows umstellen; danach funktionaler Browser-Smoke der Telefonie- UND Meeting-Audio-Pfade (nicht nur Hydration). Playbook security-headers-live-smoke.md um Feature-Flow-Coverage (connect-src/permissions je genutztem Endpoint) ergänzen. Fix-Slice: BL-520.
...
- Next Action: MeetingPrepCard res.error auswerten (Toast/Meldung statt No-Op); zusätzlich in getNextMeetingWithContext keine unsichtbare rohe contact_id durchreichen (auf null mappen, wenn der Join leer ist). Fix-Slice: BL-520.
...
- Next Action: In checkConsentStatus einen Count-Guard ergänzen — returned-Row-Count < contactIds.length ⇒ fail-closed (allGranted=false, fehlende IDs als missing behandeln). Damit ist die Lib unabhängig vom Caller sicher. Fix-Slice: BL-520.
