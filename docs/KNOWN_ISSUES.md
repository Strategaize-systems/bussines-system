...
- Next Action: TEIL-BEHOBEN (Ansatz a, DEC-307). MT-4 CODE-DONE 2026-07-08 (MIG-055, RPT-679): UPDATE-`WITH CHECK` der 9 Tabellen zurück auf OR + changed-FK-only BEFORE-UPDATE-Trigger (SECURITY INVOKER, service_role-aware); INSERT bleibt AND. OQ-A1 live in-tx-Probe gegen Coolify-DB 8/8 PASS (mixed-owner-Status-Update entsperrt, Cross-Tenant-FK-Injection UPDATE+INSERT weiter BLOCK). MT-5 CODE-DONE 2026-07-08 (RPT-681, commit `ca81ca5`): die 4 Call-Sites prüfen jetzt Fehler + kompensieren — followup-actions approveFollowup (approve-Update-Check + Task-Insert-Rollback auf pending), fit-assessment/signal-actions createSignalForActivity (RLS-42501→klare Meldung), emails/imap-actions assignEmailToContact (Thread-Update-Fehler→Message-contact_id-Kompensation), meetings/actions createMeeting (calendar_events-Fehler→Teil-Erfolg-Meldung). Kompensations-Branches sind Multi-User-latent (heute is_admin-maskiert). OFFEN für Status-Resolution: (1) Live-Apply MIG-055 im /deploy (aktuell nur in-tx-verifiziert, nicht persistent); (2) MT-6 (ISSUE-139) + Gesamt-/qa V8.17 zum Slice-Abschluss. Fix-Slice: BL-520 / SLC-915.
