...
- Next Action: Entscheiden, ob die AND-Conjunction für UPDATE zu streng ist — Optionen: (a) UPDATE-WITH-CHECK nur die GEÄNDERTEN FK-Spalten prüfen (nicht die unveränderten) via Trigger/Spalten-Vergleich; (b) UPDATE bei unveränderten FKs auf USING-Konsistenz (OR) zurückführen und nur INSERT/FK-Change streng halten; (c) mixed-owner-Rows durch Bulk-Reassign-Härtung verhindern (Parent-FKs mit-reassignen). Zusätzlich: die 4 Call-Sites mit verschlucktem Insert/Update-Fehler auf Fehlerprüfung + Kompensation umstellen. Gebündelt mit dem Multi-User-Schritt (nach BL-519). Fix-Slice: BL-520.
