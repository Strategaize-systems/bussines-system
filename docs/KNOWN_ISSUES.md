...
### ISSUE-089 — GoTrue signInWithPassword broken fuer bestimmte Bestands-User (Diagnose-Scope verschoben nach V8.13 Investigation)
- Status: open
- Severity: High (Pre-Customer-Live Blocker)
- Area: Auth / GoTrue / Self-Hosted
- Summary: V8.13 Investigation 2026-06-03 (RPT-573) hat die urspruengliche Hypothese **"alle Bestands-User broken"** als **zu breit** widerlegt: Fresh-Signup + Sign-In Test gegen Production-BS PASS (Test-User angelegt + signInWithPassword sofort danach → access_token returned). GoTrue v2.160 funktioniert grundsaetzlich. Aber: Bestehende User richard + qa-admin angeblich broken — Symptom ist `{"code":400,"error_code":"invalid_credentials"}` selbst nach SQL-Direkt-crypt()-Password-Reset (per RPT-569 verified `SELECT crypt('pwd', encrypted_password) = encrypted_password` returnt TRUE auf DB-Level). 6 Hypothesen zur weiteren Investigation in SLC-895: H-1 GoTrue v2.160 Bcrypt-Cost-Strenge ($2a$06$ rejected — UNWAHRSCHEINLICH weil qa-admin $2a$06$ gestern erfolgreich), H-2 auth.identities-Drift fuer alte User (fehlende provider-Row?), H-3 User-Side falsches Passwort im Kopf, H-4 Coolify-Redeploy `encrypted_password`-Corruption, H-5 GoTrue v2.160 Pre-Check der bei alten Usern fehlt, H-6 Browser-Cookie/CSRF-Issue. Aktueller Stand: 4 User in auth.users (richard aud='', qa-admin/qa-member/qa-teamlead aud='authenticated', alle email_confirmed_at=t, banned_until=NULL, deleted_at=NULL). Cross-Repo-Status: OP gleiche GoTrue v2.160 — Bug-Reproduzierbarkeit auf OP noch nicht getestet.
- Impact: Production-Login fuer Founder + qa-Test-User aktuell unmoeglich. Pre-Customer-Live blockierend. Live-Smoke V8.10 hat Workaround via Signup-Pfad genommen (`POST /auth/v1/signup` → neuer User mit funktionalem JWT → Cookie-Session-Test).
- Workaround: Brand-neue Signup-User koennen sich sofort einloggen. Bestehende User Workaround unklar bis Investigation-Resolution.
- Discovery: 2026-06-03 BS V8.10 Gesamt-/qa Live-Smoke (RPT-569). GoTrue-Image `supabase/gotrue:v2.160.0`. **Diagnose verfeinert** in V8.13 Investigation 2026-06-03 (RPT-573).
- V8.13 Resolution-Plan: SLC-895 Investigation (1-2h Reproducer + identities-Cross-Check + Changelog-Recherche + Hypothese-Resolution) + Fix-Implementation (30-60min, Pfad abhaengig von Root-Cause: Migration / Admin-API-Reset / ENV-Anpassung / User-Anleitung) + Founder Live-Verify.
- Next Action: (a) **V8.13 SLC-895 ausfuehren**. (b) MT-1 Reproducer-Script via qa-admin crypt()-Reset. (c) MT-2 auth.identities-Drift-Audit Fresh-Signup vs Bestand. (d) MT-3 GoTrue v2.160 Changelog Web-Recherche. (e) MT-4 Hypothese-Resolution. (f) MT-5 Fix-Implementation. (g) MT-6 Founder Live-Verify. (h) MT-7 Cross-Repo-OP-Test ob auch betroffen.
