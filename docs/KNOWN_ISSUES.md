...
### ISSUE-088 — Storage-API Self-Access INSERT fehlende GRANTs (NICHT RLS — Storage v1.11.13 verkleidet 42501 als RLS-Error)
- Status: open
- Severity: High
...
- Impact: Production-Document-Uploads via `supabase.storage.from(bucket).upload(...)` scheitern fuer alle user-scoped Buckets in BS + OP. Founder kann via UI keine Documents anlegen. Internal-Test-Mode aktuell nicht direkt blockierend (kein Customer affected). Bei Customer-Go-Live blockierend. RLS-Defense (SLC-893 documents_user_* Policies) ist KORREKT — sie wird nur gar nicht erst evaluiert weil GRANT-Check vorgelagert failt.
- Workaround: Server-Actions koennten temporaer `createAdminClient` statt `createClient` nutzen (service_role BYPASSRLS, hat alle GRANTs). Aber das hebt RLS-Defense auf. Besser: V8.13 SLC-894 Quick-Fix (idempotente Standard-GRANTs setzen).
...
- V8.13 Resolution-Plan: SLC-894 BS storage-Schema GRANTs Hotfix (~1h, MIG-043 idempotente GRANT SELECT/INSERT/UPDATE/DELETE auf storage.* fuer authenticated+anon) → Live-Verify Storage-INSERT funktioniert wieder. RLS-Defense bleibt aktiv (SLC-893 documents_user_* greift weiter fuer Cross-Tenant-Defense). Cross-Repo-Mirror in OP-Repo nach BS-Resolution.
- Next Action: (a) **V8.13 SLC-894 ausfuehren** (~1h Quick-Fix). (b) Cross-Repo OP V8.0.2 SLC-XXX gleicher Fix in OP-Repo. (c) V8.14 Container-Upgrade auf v2.186 + v1.44.2 (separater Sprint, Storage v1.44+ vermutlich hat den Misleading-Error-Message-Bug auch gefixt). (d) Dev-System Pattern-Library-Entry `12-storage-grants-init-sql.md` als Re-Inzidenz-Defense.
