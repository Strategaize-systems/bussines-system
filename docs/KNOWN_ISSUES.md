...
### ISSUE-094 — document-actions umgeht Klasse-C-RLS via createAdminClient fuer documents (Klasse-C-Block-3-Cron-Audit-Finding) [RESOLVED 2026-06-09]
- Status: resolved
- Severity: Medium
- Resolution: V8.12 SLC-906 MT-5 (2026-06-09). No-Op-Befund: document-actions.ts wurde bereits in V8.10 SLC-893 MT-4 (Commit 14567b1) auf User-Client umgestellt. Alle 4 Operations (getDocuments/uploadDocument/deleteDocument/getDocumentUrl) nutzen createClient(). RLS Klasse-C documents_table_* Multi-Parent OR-EXISTS greift wie designed. 4 Regression-Tests (cockpit/src/lib/actions/document-actions.test.ts) verhindern Re-Introduction von createAdminClient. ISSUE-094-Description war stale.
...
### ISSUE-093 — insight-actions umgeht Klasse-C-RLS via createAdminClient fuer ai_action_queue + ai_feedback (Klasse-C-Block-3-Pendant) [RESOLVED 2026-06-09]
- Status: resolved
- Severity: Medium
- Resolution: V8.12 SLC-906 MT-4 (2026-06-09). cockpit/src/lib/actions/insight-actions.ts: 4 Server-Actions (getPendingInsights/approveInsightAction/rejectInsightAction/batchApproveInsightActions) auf User-Client umgestellt (kein createAdminClient mehr). ai_action_queue + ai_feedback + activities INSERTs/UPDATEs alle via User-Client; getCurrentUserId-Helper geloescht (inline supabase.auth.getUser). RLS Klasse-C polymorph 5-Wege-EXISTS greift fuer Member-eigene Insights + Admin. Deviation vs Slice-Spec-Text 'assertRole([admin])': spec-Wording wuerde Members vom Mein-Tag-Approve aussperren — daher User-Client (matched ISSUE-093 Next-Action 'Option A preferred'). 7 Vitest-Tests GREEN (cockpit/src/lib/actions/insight-actions.test.ts).
...
### ISSUE-092 — emails-Compose send-action umgeht Klasse-C-RLS via createAdminClient fuer email_attachments-Bulk-Insert (Klasse-C-Block-2-Pendant) [RESOLVED 2026-06-09]
- Status: resolved
- Severity: Medium
- Resolution: V8.12 SLC-906 MT-3 (2026-06-09). cockpit/src/app/(app)/emails/compose/send-action.ts: email_attachments-Bulk-INSERT (L196-209) auf User-Client umgestellt (createAdminClient-Import entfernt). RLS Klasse-C email_attachments_insert greift fuer den Composing-User da die emails-Row zuvor mit owner_user_id=profile.user_id via sendEmailWithTracking angelegt wird. 2 Vitest-Smoke-Tests GREEN (cockpit/src/app/(app)/emails/compose/send-action.test.ts). Spec-Drift dokumentiert: Slice-Spec MT-3 erwaehnte Storage-Cleanup-Branch, real existiert nur 1 createAdminClient-Site (email_attachments).
...
### ISSUE-091 — deal-products Server-Actions umgehen Klasse-C-RLS via createAdminClient ohne is_admin()-Pre-Check (Klasse-C-Pendant zu ISSUE-090) [RESOLVED 2026-06-09]
- Status: resolved
- Severity: Medium
- Resolution: V8.12 SLC-906 MT-2 (2026-06-09). cockpit/src/app/actions/deal-products.ts: alle 4 Operations (listDealProducts/assignProduct/updateDealProduct/removeProduct) auf User-Client umgestellt (createAdminClient-Import entfernt). RLS Klasse-C deal_products_{select,insert,update,delete} `EXISTS(SELECT 1 FROM deals d WHERE d.id=deal_id AND can_see_owner(d.owner_user_id)) OR is_admin()` greift fuer Member-eigene Deals + Admin. 9 Vitest-Tests GREEN (cockpit/src/app/actions/deal-products.test.ts).
...
### ISSUE-090 — products Server-Actions umgehen Klasse-B-RLS via createAdminClient ohne is_admin()-Pre-Check [RESOLVED 2026-06-09]
- Status: resolved
- Severity: Medium
- Resolution: V8.12 SLC-906 MT-1 (2026-06-09). cockpit/src/app/actions/products.ts: 3 Write-Funcs (createProduct/updateProduct/archiveProduct) bekommen `await assertRole(["admin"])`-Pre-Check vor createAdminClient (Defense-in-Depth). 2 Read-Funcs (listProducts/listProductCategories) auf User-Client umgestellt (RLS Klasse-B SELECT USING(true) lasst alle authenticated lesen). RLS Klasse-B INSERT/UPDATE/DELETE is_admin() bleibt als DB-Layer-Second-Line. 9 Vitest-Tests GREEN (cockpit/src/app/actions/products.test.ts).
...
- Resolution: V8.13 SLC-895 (2026-06-03). MIG-044 `044_v813_auth_users_aud_normalize.sql` idempotent applied auf BS Production (`supabase-db-k9f5pn5upfq7etoefb5ukbcg-065643061649`) via SSH+base64+psql als `postgres`-Superuser. **Root-Cause IDENTIFIZIERT + REPRODUZIERT in MT-1 A/B-Test** (slices/SLC-895-investigation-results.md): GoTrue v2.160.0 `internal/api/token.go::ResourceOwnerPasswordGrant` sucht im signInWithPassword-Endpoint per `FindUserByEmailAndAudience(email, audience)` mit `audience`-Parameter aus Request. Supabase-JS-Client setzt diesen NICHT default → Default ist `''` (leer). Filter wird zu `WHERE instance_id=? AND LOWER(email)=? AND aud='' AND is_sso_user=false`. User mit `auth.users.aud='authenticated'` werden vom Default-Lookup **NICHT** gefunden → 400 invalid_credentials OHNE Passwort-Hash-Check. **Hypothesen H-1..H-6 alle widerlegt** (H-1 Bcrypt-Cost ist NoOp in v2.160 Code, H-2 identities-Rows alle vorhanden + identisch strukturiert, H-3 User-Side reproducer-widerlegt, H-4 crypt-Hash-Verify TRUE, H-5 Pre-Check existiert nicht in v2.160 Code, H-6 curl-Reproducer ohne Browser bestaetigt Server-Side-Issue). **Neue Hypothese H-7** bestaetigt: `aud='authenticated'` blockt Default-Lookup. Reproducer 2026-06-03: Test A qa-admin mit aud='authenticated' + crypt-Reset → HTTP 400; Test B qa-admin mit aud='' + gleicher crypt-Reset → HTTP 200 + access_token. Fix-Migration: `UPDATE auth.users SET aud='' WHERE aud='authenticated';` (3 Rows updated: qa-admin/qa-member/qa-teamlead). richard war NICHT betroffen (hatte `aud=''` schon, vor-V7-User). JWT-Output-Claim `aud=''` ist ohnehin leer und wird nicht von BS Cockpit (`@supabase/auth-helpers-nextjs`) oder PostgREST validiert. Vitest-Schema-Verification `cockpit/__tests__/migrations/044-v813-auth-users-aud-normalize.test.ts` (5 Tests: 0 User mit aud='authenticated', alle aktiven Non-SSO-User aud='', role-Spalte unangetastet, instance_id unangetastet, Migration ist idempotent) per node:20 Sidecar im business-net 5/5 PASS in 33ms. Post-Apply Live-Smoke: (a) curl signInWithPassword qa-admin (post-MIG-044 + crypt-Reset Cost-10 + aud='') → HTTP 200 access_token + role='authenticated' JWT-claim. (b) **Founder Richard Live-Verify Browser-Login auf https://business.strategaizetransition.com mit richard@bellaerts.de PASS** 2026-06-03.
- Area: Auth / GoTrue / Self-Hosted Container-Versions-Drift / Manual-User-Seed-aud-Drift
- Summary: V8.13 Investigation 2026-06-03 (RPT-573) hat die urspruengliche Hypothese **"alle Bestands-User broken"** als **zu breit** widerlegt: Fresh-Signup + Sign-In Test gegen Production-BS PASS. SLC-895 Investigation 2026-06-03 (slices/SLC-895-investigation-results.md) hat dann via Reproducer-A/B-Test die **echte Wurzel** identifiziert: GoTrue v2.160's `FindUserByEmailAndAudience` matched nur User mit `aud=''` beim Default-signInWithPassword-Lookup. Die 3 manuell via SQL angelegten qa-User (qa-admin, qa-member, qa-teamlead) hatten explizit `aud='authenticated'` gesetzt — vermutlich aus einem alten Setup-Skript ausserhalb des Repos — und waren dadurch ausgeschlossen. Richard war pre-V7-User mit `aud=''` (Fresh-Signup-Default in v2.160) und hatte das Problem nicht — sein "broken Login" war vermutlich H-3 (falsches Passwort im Kopf) oder Browser-Session-Confusion.
- Impact: Production-Login fuer 3 qa-Test-User (qa-admin, qa-member, qa-teamlead) war broken. Founder richard@bellaerts.de war NICHT betroffen. Pre-Customer-Live-relevant fuer kuenftige Manual-User-Seeds: wenn neue Onboarding-User via SQL-Direkt-Insert angelegt werden, MUSS `aud=''` gesetzt sein (nicht `aud='authenticated'`).
- Discovery: 2026-06-03 BS V8.10 Gesamt-/qa Live-Smoke (RPT-569). GoTrue-Image `supabase/gotrue:v2.160.0`. **Diagnose verfeinert** in V8.13 Investigation 2026-06-03 (RPT-573). **Root-Cause identifiziert** in V8.13 SLC-895 Investigation 2026-06-03 (slices/SLC-895-investigation-results.md).
- Next Action: (a) Cross-Repo OP V8.0.x SLC-XXX gleicher MIG-044-Fix-Mirror in OP-Repo per `docs/CROSS_REPO_V813_STORAGE_GRANTS.md`-Vorlage (jetzt erweitert um MIG-044). OP muss erst gepruefft werden ob aud='authenticated'-Drift existiert. (b) V8.14 Container-Upgrade auf v2.186+ — moeglicherweise hat v2.186 relaxed-Lookup der diesen Bug downstream-haerter. (c) Pattern-Library-Entry `13-auth-users-aud-default.md` in Dev-System ergaenzen: bei jedem manuellen User-Seed via SQL MUSS `aud=''` gesetzt sein (oder Spalte weglassen → Default NULL ist auch akzeptabel). (d) Re-Inzidenz-Defense: bei naechstem Manual-User-Seed-Operation ein Schema-Check-Script laufen lassen.

### ISSUE-088 — Storage-API Self-Access INSERT fehlende GRANTs (NICHT RLS — Storage v1.11.13 verkleidet 42501 als RLS-Error) [RESOLVED 2026-06-03]
- Status: resolved
- Severity: High
- Resolution: V8.13 SLC-894 (2026-06-03). MIG-043 `043_v813_storage_schema_grants.sql` idempotent applied auf BS Production (`supabase-db-k9f5pn5upfq7etoefb5ukbcg-065643061649`) via SSH+base64+psql als `postgres`-Superuser. Setzt fuer Rollen `authenticated`+`anon`: (a) `GRANT SELECT,INSERT,UPDATE,DELETE ON ALL TABLES IN SCHEMA storage` (additiv, kein REVOKE), (b) `GRANT USAGE,SELECT ON ALL SEQUENCES` (defensiv, aktuell 0 Sequences im storage-Schema), (c) `ALTER DEFAULT PRIVILEGES IN SCHEMA storage` fuer `postgres` + `supabase_storage_admin` (Future-Proofness bei Container-Upgrades), (d) `NOTIFY pgrst, 'reload schema'`. Pre-Apply-Audit dokumentiert in `slices/SLC-894-pre-apply-audit.md` (authenticated+anon hatten nur SELECT auf alle 5 storage-Tables). Post-Apply-Verify: psql-Query zeigt alle 4 CRUD-Privileges fuer authenticated+anon auf storage.objects (8 Rows wo vorher 2). Vitest-Schema-Verification `__tests__/migrations/043-v813-storage-schema-grants.test.ts` (5 Tests: GRANTs+DEFAULT-PRIVILEGES+RLS-Defense+service_role-unchanged) per node:20 Sidecar im business-net 5/5 PASS (RED-vor-Apply 3/5 FAIL bestaetigte Bug-Detection). Live-Smoke 4/4 PASS via curl+self-signed-HS256-JWT (umgeht ISSUE-089): (1) User-A INSERT eigener Pfad `documents/<user-A-uuid>/test-smoke-mt4-*.txt` HTTP 200 + Key returned → ISSUE-088 closed. (2) User-A INSERT fremder User-B-Pfad HTTP 400 mit "row-level security policy" Body → RLS-Defense aus MIG-041 (documents_user_insert mit first-path-segment-Filter) bleibt aktiv (AC-894-4). (3) User-A SELECT eigener Pfad HTTP 200 + Body match. (4) Cleanup DELETE HTTP 200 → keine Test-Pollution. service_role unangetastet, RLS-Policies aus MIG-041 unangetastet, kein Schema-Bruch.
- Area: Storage / GRANT / Self-Hosted Container-Versions-Drift
- Summary: V8.13 Investigation 2026-06-03 (RPT-573) hat Root-Cause IDENTIFIZIERT: Storage-Container-Logs zeigen tatsaechlichen PostgreSQL-Error `42501 insufficient_privilege` aus `aclchk.c:3650` (PG Access-Control-Layer = **GRANT-Check**), NICHT RLS-Policy-Violation. Storage-Service v1.11.13 castet alle 42501-Errors zu Misleading-Message "new row violates row-level security policy" — Upstream-Bug. **Echte Wurzel:** Rolle `authenticated` hat fehlende Default-GRANTs auf `storage.*`-Tabellen. Cross-Repo-Versions-Audit zeigt: BS nur `authenticated|SELECT` (kein INSERT/UPDATE/DELETE), OP gar keine GRANTs, IS+ImSch volle CRUD. Korrelation mit Container-Version: BS+OP auf GoTrue v2.160 + Storage v1.11.13 (alte Versionen, Init-Script setzt GRANTs nicht), IS+ImSch auf v2.186 + v1.44.2 (neuere Init-Scripts setzen GRANTs automatisch). **Cross-Repo systemisch** — OP gleicher Bug, noch nicht entdeckt weil OP Storage moeglicherweise nicht im Hot-Path nutzt.
- Impact: Production-Document-Uploads via `supabase.storage.from(bucket).upload(...)` scheiterten fuer alle user-scoped Buckets in BS + OP. Founder konnte via UI keine Documents anlegen. RLS-Defense (SLC-893 documents_user_* Policies) ist KORREKT — sie wurde nur gar nicht erst evaluiert weil GRANT-Check vorgelagert failte. Nach MIG-043: GRANT-Check passt → RLS-Defense greift wie designed.
- Discovery: SLC-893 MT-6 Live-Smoke 2026-06-03 (RPT-568) + Gesamt-/qa Live-Smoke 2026-06-03 (RPT-569) → initial als RLS-Bug fehldiagnostiziert + V8.13 Investigation 2026-06-03 (RPT-573) → Root-Cause als GRANT-Bug verifiziert via Storage-Container-Log + Cross-Repo-Versions-Audit + GRANT-Matrix-Check ueber 4 Server.
- Next Action: (a) Cross-Repo OP V8.0.2 SLC-XXX gleicher Fix in OP-Repo per `docs/CROSS_REPO_V813_STORAGE_GRANTS.md`-Vorlage — OP ist sogar schlimmer (KEINE GRANTs vs BS nur SELECT). (b) V8.14 Container-Upgrade auf v2.186 + v1.44.2 (separater Sprint, Storage v1.44+ vermutlich hat den Misleading-Error-Message-Bug auch gefixt). (c) Dev-System Pattern-Library-Entry `12-storage-grants-init-sql.md` als Re-Inzidenz-Defense bei Coolify-Storage-Container-Recreates.
...
### ISSUE-135 — Kein Self-Service-Passwort-vergessen-Flow (User-Lockout ohne Admin-Eingriff)
- Status: open
- Severity: Medium
- Area: Auth / UX / Pre-Multi-User-Gate
- Summary: Die Login-Seite hat keinen "Passwort vergessen"-Link und es existiert kein Reset-Pfad im Code (kein `resetPasswordForEmail`/GoTrue-recovery-Aufruf; `src/app/auth/` enthält nur `callback` + `set-password` + `signout` = Invite-Flow). Wer sein Passwort vergisst, ist ausgesperrt, bis jemand mit Server-Zugriff eingreift. Founder-Befund 2026-07-06, code-verifiziert über alle 4 Strategaize-Repos (OP hat den Flow seit SLC-186 in-flight).
...
### ISSUE-138 — CSP-Enforce (SLC-910 Phase-B) blockt SIP-Telefonie-WebSocket + Mikrofon
- Status: resolved
- Severity: High
- Area: Security-Header / CSP / Telefonie / WebRTC
- Summary: Der V8.16-Flip von `Content-Security-Policy-Report-Only` auf enforced `Content-Security-Policy` (next.config.ts:19) aktiviert eine `connect-src`-Allowlist (csp.ts:19-26), die den SIP-WebSocket-Endpoint NICHT enthält. Der In-App-Softphone-Hook (use-sip-phone.ts:95) öffnet eine sip.js-Transport-WebSocket zu `wss://sip.strategaizetransition.com/ws` (aus calls/actions.ts:265-268, Fallback bei leerem `NEXT_PUBLIC_SIP_DOMAIN` — live leer). Browser erzwingen `connect-src` auch für WebSockets → connect() wirft → use-sip-phone.ts:117 setzt `sip-connect-failed`. Zusätzlich blockt `Permissions-Policy: microphone=()` (live verifiziert) das Mikrofon komplett → auch bei erfolgreicher WSS-Verbindung kein Call-Audio; potenziell auch Jitsi-Meeting-Audio betroffen (zu verifizieren, falls Jitsi als iframe auf der App-Origin eingebettet ist). Asterisk/SIP-Infra ist deployed + konfiguriert (SIP_TRUNK_ENABLED, ASTERISK_WEBRTC_PASSWORD live gesetzt) → Feature ist scharf. Gefunden im lokalen High-Effort-Review über c9efb41..HEAD (RPT-672, Klasse security-headers-live-smoke-Trugschluss).
- Impact: Click-to-Call-Telefonie in Production gebrochen, sobald genutzt. Unter Report-Only funktionierte identisch (Violations nur gemeldet), daher von allen QA-Stufen + csp-check.mjs übersehen — der Probe deckt nur Hydration auf /login + /mein-tag, nicht die Telefonie-/Meeting-Feature-Flows. Heute im Single-Founder-Internal-Test-Mode nur bei aktiver Telefonie-Nutzung spürbar; Multi-User-blockierend.
- Workaround: CSP-Header temporär zurück auf `Content-Security-Policy-Report-Only` (next.config.ts:18 Key-Flip + Redeploy) stellt Telefonie sofort wieder her, verliert aber die Enforce-Härtung von SLC-910.
- Next Action: CODE-DONE 2026-07-08 (SLC-915 MT-1, RPT-677) — `connect-src` um `wss://${NEXT_PUBLIC_SIP_DOMAIN||sip.strategaizetransition.com}` erweitert, `Permissions-Policy` `microphone=()`→`microphone=(self)` (`camera=()` bleibt: Jitsi = separater Origin via window.open, Step-0 DRIFT-1). csp.test.ts 12/12, volle Suite grün, next build ✓. **Bleibt open bis /deploy:** funktionaler Telefonie-/Meeting-Browser-Smoke (WSS verbindet + Mikro + 0 connect-src/Permissions-Console-Errors auf dem Call-Flow, nicht nur Hydration) — AC-915-1/2 PENDING-LIVE. Fix-Slice: BL-520. LIVE-VERIFIED 2026-07-09 (/deploy V8.17, RPT-687): enforced CSP-Header traegt `wss://sip.strategaizetransition.com` in connect-src + `Permissions-Policy: microphone=(self), camera=()`; Call-Surface 0 connect-src/Permissions-Console-Errors. CSP-Block aufgehoben. RESOLVED.

### ISSUE-139 — CSP-Enforce blockt Remote-Images in der Inbound-E-Mail-Vorschau
- Status: resolved
- Severity: Medium
- Area: Security-Header / CSP / E-Mail-Viewer
- Summary: `img-src 'self' data: blob:` (csp.ts:36, kein `https:`) gilt via CSP-Vererbung auch für die srcDoc-Sandbox des E-Mail-Viewers (email-html-iframe.tsx:74-78 — about:srcdoc-Dokumente erben die CSP des Embedders). Der Sanitizer (sanitize-email-html.ts:39-40) lässt `https:`-img-src bewusst durch, d.h. Remote-Bilder in empfangenen IMAP-Mails (Logos, Signaturen, Newsletter-Bilder) überleben die Sanitisierung und wurden unter Report-Only geladen — unter Enforce werden sie geblockt. Gefunden RPT-672.
- Impact: Empfangene HTML-Mails rendern Remote-Bilder als Broken-Image, Layout kollabiert teilweise (measureAndResize misst das bildlose Dokument). Silent — csp-check.mjs probt den Viewer nicht. Bug-Reports landen wahrscheinlich fälschlich beim IMAP-Sync statt beim CSP-Flip. Privacy-Upside (Tracking-Pixel-Block), aber undokumentiert.
- Next Action: DEC-306 accepted (Block-by-default + opt-in route-scoped CSP, kein Proxy). MT-6 CODE-DONE 2026-07-08 (RPT-682, commit `9c22374`): `sanitizeEmailHtml({blockRemoteImages})` strippt http(s)-img-src (cid:/data:image bleiben) + `hasRemoteImages`-Pre-Scan; neue Route `GET /api/emails/[id]/body` (RLS-User-Client, `email_messages.body_html`, route-scoped CSP `default-src 'none'; img-src https: data: blob: cid:; ...`, kein Proxy); `EmailHtmlIframe` 2-Zustaende (blocked srcDoc+Banner „Bilder laden" / loaded iframe-src→Route + sandbox=allow-same-origin). Suite 1636/1636, tsc0/build PASS. OFFEN für Status-Resolution: Browser-Feature-Flow-Smoke (Bild-Block/Load + Hoehenmessung + RLS-404) im Gesamt-/qa + /deploy (OQ-A2 Live-Confirm body_html). Fix-Slice: BL-520 / SLC-915. LIVE-VERIFIED 2026-07-09 (/deploy V8.17, RPT-687): nach ISSUE-145-Hotfix rendern im echten Viewer 71/71 Remote-Bilder (Hoehe 2883px, 0 CSP-Violations), Block-State + Banner korrekt. RESOLVED.

### ISSUE-140 — MIG-054 AND-Conjunction friert legitime mixed-owner-Rows für Non-Admins ein (Multi-User-latent)
- Status: resolved
- Severity: Medium
- Area: Security / RLS / Multi-User-Readiness
- Summary: MIG-054 stellt INSERT- UND UPDATE-`WITH CHECK` der 9 Multi-Parent-Klasse-C-Tabellen von OR auf AND-Conjunction um (jeder gesetzte FK muss can_see_owner-sichtbar sein), lässt aber USING (SELECT/UPDATE/DELETE) als OR. Postgres prüft WITH CHECK gegen die volle NEUE Row inkl. unveränderter FK-Spalten → ein reines Status-Update (z.B. completeTask, aufgaben/actions.ts:156) auf einer Row mit einem sichtbaren + einem unsichtbaren non-NULL Parent schlägt für Non-Admins mit 42501 fehl. Mixed-owner-Rows sind real erzeugbar: Bulk-Reassign (SLC-707, lib/team/bulk-reassign.ts) verschiebt eine Activity/Task zu User B, während referenzierte Contact/Company bei User A bleiben; plus Alt-Rows aus der OR-Ära. Betroffene Insert-/Update-Pfade mit zusätzlich verschlucktem Fehler: followup-actions.ts:126 (ai_action_queue schon approved, Task-Insert scheitert → approved ohne Task), signal-actions.ts:73 (rohe RLS-Fehlermeldung an UI), imap-actions.ts:135 (Thread-Update silent-fail → Message/Thread-Divergenz), meetings/actions.ts:228 (calendar_events-Insert-Fehler verworfen). Gefunden RPT-672.
- Impact: Heute vollständig durch is_admin()-Single-User-Betrieb maskiert (0 Impact im Founder-Mode). Beim geplanten Multi-User-Start: rohe RLS-42501-Fehler bei Alltags-Updates über 9 Tabellen + inkonsistente Zustände (approved-ohne-Task, Message/Thread-Divergenz). Kein Datenverlust, Admin kann weiter updaten. Multi-User-Blocker.
- Next Action: RESOLVED 2026-07-08 im /deploy V8.17 (RPT-686). MIG-055 **live-appliziert** gegen die Coolify-DB (2× idempotent EXIT=0) + Post-Apply-Struktur (9 `*_changed_fk_guard`-Trigger, `tasks_update`=OR, `tasks_insert`=AND-strikt, Functions SECURITY INVOKER) + **Semantik-Probe gegen die persistente DB 4/4 PASS** (mixed-owner-Status-Update entsperrt rows=1 = MIG-054-Freeze behoben, FK-Change-auf-unsichtbar BLOCK 42501, legit FK-Change-auf-sichtbar PASS, INSERT-strikt Cross-Tenant BLOCK); Persistenz nach Compose-Recreate re-verifiziert (9 Trigger). Die 4 Call-Site-Kompensationen (MT-5, `ca81ca5`) sind code-done und Multi-User-latent (heute is_admin-maskiert, 0 Impact im Founder-Mode) → Live-Exercise erst beim Multi-User-Start; 3 weitere Twins derselben Klasse in ISSUE-144 (Low, deferred). Fix-Slice: BL-520 / SLC-915.

### ISSUE-141 — startMeeting-Ownership-Gate erzeugt Silent-Dead-Button in Mein Tag
- Status: resolved
- Severity: Medium
- Area: Frontend / Auth-Gate / Regression
- Summary: `getNextMeetingWithContext` (mein-tag/actions.ts:449) fällt auf die rohe meeting.contact_id zurück, wenn der eingebettete contacts-Join RLS-leer ist (`contactId: contact?.id ?? (meeting as any).contact_id ?? null`) — diese ID ist für den User per Definition unsichtbar. MeetingPrepCard (mein-tag-client.tsx:571-576) reicht sie unverändert an das neue V8.16-startMeeting-Ownership-Gate durch, das jetzt fail-closed `{error: "…Kontakte…nicht zugreifbar"}` zurückgibt — aber MeetingPrepCard wertet nur res.hostRedirectUrl aus, nie res.error. Vor V8.16 lief der Contact-Read via createAdminClient und das Meeting startete trotzdem. Gefunden RPT-672.
- Impact: Eigenes geplantes Meeting mit RLS-unsichtbarem contact_id (z.B. nach Reassignment) → "Meeting starten" tut sichtbar nichts (kein window.open, keine Meldung, kein Meeting, keine Einladung). Silent-Dead-Button-Regression, von den neuen Tests nicht abgedeckt (testen nur die Server-Action direkt). Heute selten (Single-User), Multi-User-verstärkt.
- Next Action: CODE-DONE 2026-07-08 (SLC-915 MT-3, RPT-678) — `getNextMeetingWithContext` mappt `contact?.id ?? null` (roher meeting.contact_id-Fallback entfernt, Grep 0 Treffer); MeetingPrepCard wertet `res.error` aus (lokaler useState + `role="alert"`-Inline-Banner statt No-Op). tsc/build clean, Suite grün. **Bleibt open bis /deploy:** Browser-Smoke „fehlgeschlagener Meeting-Start → sichtbares Banner" (AC-915-4 Interaktions-Teil PENDING-LIVE, kein RTL-Test ergänzt). Fix-Slice: BL-520. DEPLOYED 2026-07-09 (/deploy V8.17): code-verifiziert (res.error-Handling + `contact?.id ?? null`, Grep+Unit); defensiver Anzeigepfad, Fehler-Interaktion nicht erzwungen. RESOLVED.

### ISSUE-142 — checkConsentStatus ist mit RLS-Client fail-OPEN (Recording ohne Consent-Beweis möglich)
- Status: resolved
- Severity: Medium
- Area: Security / DSGVO / §201 StGB / Consent-Gate
- Summary: `checkConsentStatus` (lib/meetings/consent-check.ts:55-64) klassifiziert nur zurückgelieferte Rows und vergleicht die Row-Zahl nie mit contactIds.length → RLS-weggefilterte Kontakte landen weder in granted noch in missing, und `allGranted = missing.length===0` wird fail-open true. Der V8.16-Diff reicht jetzt den RLS-scoped User-Client durch (meetings.ts:147). Zwischen dem Visibility-Pre-Gate (meetings.ts:97-107) und dem Consent-Read (:147) liegt der Meeting-INSERT (:126-140) = TOCTOU-Fenster: ein paralleles Owner-/Team-Reassignment eines Kontakts entfernt ihn aus der RLS-Sicht → Consent-Read liefert weniger Rows → missing=[] → recording_status=pending, Aufzeichnung startet, obwohl der Kontakt nie Consent erteilt hat. Shared §201-Consent-Gate-Lib: jeder künftige Caller ohne exakten Pre-Gate erbt das fail-open. Gefunden RPT-672.
- Impact: Recording ohne Consent-Beweis (§201 StGB / DSGVO-relevant) auf einem schmalen concurrent-Reassignment-Race; heutiger einziger Caller hat einen Pre-Gate, daher latent. Design-Fehler der Shared-Lib ist das eigentliche Risiko.
- Next Action: RESOLVED 2026-07-08 (SLC-915 MT-2, RPT-678). Count-Guard ergänzt: `returnedIds`-Set über die zurückgelieferten Rows; jede angeforderte contactId ohne Row ⇒ synthetischer `missing`-Eintrag (leere PII, da RLS-unsichtbar). `allGranted=missing.length===0` ist damit caller-unabhängig fail-closed. +2 Vitest (RLS-gefilterte Row / leere Query), consent-check 3→5, volle Suite grün. Reine Logik, keine Migration → code-side resolved (Meeting-Recording-Consent-Flow-Smoke läuft als Teil der Gesamt-/qa/deploy mit).

### ISSUE-143 — Consent-Banner zeigt leeren Namen für RLS-unsichtbare fehlende Teilnehmer
- Status: open
- Severity: Low
- Area: Frontend / Meetings / Consent-UI
...
### ISSUE-145 — Globale next.config-CSP überschreibt die route-scoped CSP der E-Mail-Body-Route → „Bilder laden" live gebrochen
- Status: resolved
- Severity: High
- Area: Security / CSP / E-Mail-Viewer (MT-6)
- Summary: `next.config.ts` legte die globalen SECURITY_HEADERS (CSP mit `frame-ancestors 'none'` + striktes `img-src 'self' data: blob:` + `X-Frame-Options: DENY`) via `source: "/:path*"` auf ALLE Routen — inkl. `/api/emails/[id]/body`. Next.js `headers()`-Config gewinnt gegen die im Route-Handler gesetzte route-scoped `BODY_CSP`, sodass die Response die globale CSP trug. Effekt im echten Browser: Klick auf „Bilder laden" wechselt den iframe auf `src=/api/emails/[id]/body`, aber `frame-ancestors 'none'` blockt das Einbetten komplett (Console: „Framing '…' violates frame-ancestors 'none'") → iframe bleibt leer (Höhe 80px, contentDocument null), die Mail verschwindet, Bilder rendern nie. Unit/tsc/eslint/next-build alle grün — nur im Feature-Flow-Smoke sichtbar (IMP-1401). Gefunden im /deploy V8.17 Playwright-Smoke (RPT-686-Folge), gegen echte IONOS-Mail `3523b489…`.
- Impact: MT-6-Kernfeature (opt-in Remote-Bild-Anzeige) live nicht funktionsfähig; schlimmer als Vorzustand, weil der Mail-Body im loaded-State komplett leer wird. Block-State (srcDoc + Banner) war korrekt. AC-915-7 loaded-State FAIL.
- Next Action: RESOLVED 2026-07-09 (V8.17-Hotfix, vor Records-Flip). Fix: (1) `next.config.ts` `source` auf Negative-Lookahead `"/((?!api/emails/[^/]+/body).*)"` → globale Header überspringen genau diese Route; (2) `BODY_CSP` um `frame-ancestors 'self'` ergänzt (same-origin-Einbettung erlaubt, cross-origin verboten — ersetzt das entfernte globale `X-Frame-Options: DENY`). tsc0/76 Unit-Tests grün/next build0. Live-Re-Smoke im Redeploy (unten). Kein Datenrisiko (nur Anzeige).


### ISSUE-146 — IMAP-Socket-Read-Timeout surft als uncaughtException (transiente Mailserver-Nichterreichbarkeit)
- Status: open
- Severity: Low
- Area: E-Mail / IMAP-Transport / Error-Handling
- Summary: Bei transienter Nichterreichbarkeit des IMAP-Mailservers wirft der ImapFlow-Socket `Error: read ETIMEDOUT` (errno -110, `_connId`), der NICHT im Promise-Chain der Sync-Crons (imap-sync/classify-emails) gefangen wird und als `⨯ uncaughtException` in den Prozess-Logs landet. Beobachtet im V8.17 /post-launch T+24h-Fenster (RPT-688): 18 Vorkommen in EINEM ~4-Min-Cluster (2026-07-10 06:16-06:20 UTC), danach voll erholt.
- Impact: **Kein Crash** (Next.js faengt uncaughtException; Container healthy, RestartCount=0). Kein Datenverlust — der naechste Cron-Tick synct nach. Kosmetisch/Log-Rauschen + je 1 transient-failte classify-emails/automation-runner-Execution im selben Fenster. NICHT V8.17-verursacht (IMAP-Transport von SLC-915 unberuehrt) — pre-existing, im Burn-In sichtbar geworden.
- Next Action: Bei Gelegenheit (kein Blocker): den ImapFlow-Socket-`error`/`close`-Handler bzw. den Fetch-Loop in ein try/catch mit Reconnect-Backoff kapseln (P-093 IMAP-Inbound-Sync-Pattern), sodass transiente Mailserver-Timeouts als geloggte Warnung statt uncaughtException enden. Cron-Success-Rate ist mit >99,9% (imap-sync 308/~2, classify-emails 154/1) unkritisch.
