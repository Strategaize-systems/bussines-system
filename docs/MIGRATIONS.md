...
### MIG-054 — V8.16 SLC-914 MT-2 Class-C Multi-Parent WITH-CHECK Härtung (ISSUE-132) — LIVE APPLIED 2026-07-06
- Date: 2026-07-05 (File + Live-Validierung im /backend) / **2026-07-06 LIVE appliziert im /deploy V8.16 (RPT-669)** — postgres-User via Base64, 2× idempotent EXIT=0, Post-Apply-`pg_policies`-Verify (18/18 Policies AND-Shape), DB-Verify 10/10 GREEN im node:20-Sidecar, Persistenz nach Coolify-Full-Stack-Recreate re-verifiziert. ISSUE-132 resolved.
- Scope: Additive Migration `sql/migrations/054_v816_slc914_class_c_withcheck_harden.sql`. Rewrites die INSERT- und UPDATE-`WITH CHECK` jeder genuin Multi-Parent-Class-C-Tabelle von OR zu AND-Conjunction: jeder gesetzte (non-NULL) FK muss via `can_see_owner()` sichtbar sein (`col IS NULL OR EXISTS(...)`, AND-verknüpft); plus `(mind. 1 FK non-NULL OR (alle FK NULL AND created_by=auth.uid()))`. **Schritt-0 live aus `pg_policies` bestätigter Satz (9 Tabellen, 2026-07-05):** tasks, signals, calendar_events, handoffs, cadence_enrollments, documents (mit created_by) + email_threads, referrals, email_attachments (kein created_by → all-NULL-Zweig entfällt, Orphan nur via is_admin). **Ausgeschlossen (kein Multi-FK-Gap, live bestätigt):** deal_products, auto_winloss_runs, proposal_items, proposal_payment_milestones, email_tracking_events, cadence_executions (Single-/transitiv-1-FK) + ai_action_queue, ai_feedback (polymorph über 1 entity_id/action_queue_id) + emails, fit_assessments (Direct-Owner-WITH-CHECK, kein FK-Zweig). SELECT/UPDATE-USING/DELETE unverändert OR-Logik. Idempotent (DROP POLICY IF EXISTS + CREATE). NOTIFY pgrst reload. **Validierung im /backend:** parse+execute in rolled-back Tx unter `ON_ERROR_STOP=1` (alle 18 Policies clean gegen Live-Schema), idempotent 2×, Live-Policies unverändert (has_conjunction=false persistiert).
...
- Reason: Fable-5 Re-Audit ISSUE-109 (High): `profiles_admin_update USING (is_admin() OR id=auth.uid())` laesst authenticated die eigene Row inkl. `team_id` updaten — `get_my_team_id()`/`can_see_owner()` haengen daran = Team-Isolation-Bypass. ISSUE-122 (Low): MIG-051 feuerte nur BEFORE UPDATE, INSERT-Pfad ungedeckt. Lektion IMP-1237: Column-Level-Fix muss ALLE authz-tragenden Spalten abdecken. Live-Inventar 2026-06-12 (`\d profiles` + Policy-/Helper-Read): authz-tragend sind genau `role` + `team_id`; Text-Spalte `team` ist Legacy ohne Policy-Bezug.
- Affected Areas: profiles (Trigger), Team-Isolation (profiles_select_team, can_see_owner, get_my_team_id), changeRole-/Invite-Pfade (lib/team/actions.ts + lib/auth/invite.ts — beide service_role via createAdminClient, code-verifiziert, R-913-1).
- Risk: Naiver Guard koennte legitime service_role-Pfade brechen (R-913-1) — mitigiert via `current_user = 'service_role'`-Early-Return + transaktionale Live-Semantik-Probe 2026-06-12 (BEGIN..ROLLBACK auf Coolify-DB, 7/7 PASS: T1 authenticated team_id-UPDATE BLOCKED, T2 service_role team_id-UPDATE OK, T3 role-UPDATE BLOCKED MIG-051-Regression, T4 display_name OK, T5 authenticated INSERT role=admin BLOCKED, T6 unprivilegierter INSERT OK, T7 service_role privilegierter INSERT OK; Probe-Script `sql/_probe/mig052_semantic_probe.sql`). Kein auth.users-Trigger legt profiles an (live verifiziert) — kein Signup-Pfad betroffen.
- Rollback Notes: Re-Apply von `051_v814_slc912_profiles_role_protect.sql` (stellt role-only-Guard + BEFORE-UPDATE-Trigger wieder her).
- Status: CREATED 2026-06-12, NICHT applied — Live-Apply im /deploy V8.15 (gated auf V8.14 STABLE /post-launch T+24h) via `sql-migration-hetzner.md` + DB-Verify `cockpit/__tests__/migrations/052-v815-profiles-authz-protect.test.ts` (AC-913-1).
...
### MIG-055 — V8.17 changed-FK-only UPDATE-Härtung der 9 Klasse-C-Tabellen (LIVE APPLIED 2026-07-08 im /deploy V8.17, semantisch 4/4 PASS, ISSUE-140 / DEC-307)
- Date: 2026-07-08
- Live-Apply: **APPLIED 2026-07-08** im /deploy V8.17 (RPT-686) gegen Coolify-DB `supabase-db-k9f5pn5upfq7etoefb5ukbcg` als `postgres` via Base64. 2× ausgeführt (Idempotenz), beide EXIT=0 (nur skip-NOTICEs, 0 ERROR). Post-Apply-Verify: 9 `*_changed_fk_guard`-Trigger (1/Tabelle), `tasks_update` WITH CHECK = OR-Shape, `tasks_insert` WITH CHECK = AND-strikt, changed_fk_guard-Functions SECURITY INVOKER (prosecdef=false). **Semantik-Probe gegen die persistente Live-DB 4/4 PASS** (DO-block: seed reale mixed-owner-Row via echte deals/contacts → assert als authenticated member → ROLLBACK): CHK1 mixed-owner Status-Update PASS (rows=1, MIG-054-Freeze behoben), CHK2 FK-Change-auf-unsichtbar BLOCK (42501), CHK3 legitimer FK-Change-auf-sichtbar PASS, CHK4 INSERT-strikt Cross-Tenant BLOCK. Persistenz nach Full-Stack-Compose-Recreate re-verifiziert (neuer DB-Container `-103000902567`, 9 Trigger, Volume-persistent).
- File: `sql/migrations/055_v817_slc915_class_c_changed_fk_trigger.sql` (geschrieben MT-4, 2026-07-08). DB-Verify-Test: `cockpit/__tests__/migrations/055-v817-class-c-changed-fk-trigger.test.ts` (node:20-Sidecar-Layer). OQ-A1 code-side in-tx-Probe **8/8 PASS** 2026-07-08 (RPT-679/680, Dev-IMP-1694: BEGIN → apply → seed mixed-owner → 8 Checks → ROLLBACK) — vor dem Deploy; die Live-Apply-Semantik-Probe (oben) bestätigt dasselbe Verhalten gegen die persistente DB.
- Scope: Additive Migration. Pro der 9 Multi-Parent-Klasse-C-Tabellen (tasks, signals, calendar_events, handoffs, cadence_enrollments, documents, email_threads, referrals, email_attachments): (1) UPDATE-`WITH CHECK` von MIG-054-AND zurück auf USING-konsistentes OR; INSERT-`WITH CHECK` bleibt MIG-054-AND (strikt). (2) `BEFORE UPDATE`-Trigger (SECURITY INVOKER, service_role-aware `current_user <> 'service_role'`, Scaffolding P-080 / MIG-051): pro FK-Spalte `NEW.<fk> IS DISTINCT FROM OLD.<fk> AND NEW.<fk> IS NOT NULL AND NOT EXISTS(SELECT 1 FROM <parent> p WHERE p.id = NEW.<fk> AND can_see_owner(p.owner_user_id)) → RAISE insufficient_privilege`. FK→Parent-Map: siehe ARCHITECTURE.md V8.17-Addendum.
