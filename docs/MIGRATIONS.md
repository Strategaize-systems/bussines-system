...
### MIG-055 — V8.17 changed-FK-only UPDATE-Härtung der 9 Klasse-C-Tabellen (GESCHRIEBEN + in-tx-verifiziert, Live-Apply pending /deploy, ISSUE-140 / DEC-307)
- Date: 2026-07-08
- File: `sql/migrations/055_v817_slc915_class_c_changed_fk_trigger.sql` (geschrieben MT-4, 2026-07-08). DB-Verify-Test: `cockpit/__tests__/migrations/055-v817-class-c-changed-fk-trigger.test.ts` (node:20-Sidecar-Layer, läuft im /deploy-Fenster). OQ-A1 live in-tx-Probe (Dev-IMP-1694: BEGIN → apply → seed mixed-owner → 8 Checks → ROLLBACK) gegen Coolify-DB (`supabase-db-k9f5pn5upfq7etoefb5ukbcg`) **8/8 PASS** 2026-07-08 (RPT-679): mixed-owner-Status-Update PASS (MIG-054-Freeze behoben), Cross-Tenant-FK-Injection UPDATE+INSERT weiter BLOCK (42501), service_role + admin Bypass, SECURITY-INVOKER (prosecdef=false) + parent-RLS-Sichtbarkeit im Subquery bestätigt, 9 Trigger + 9 UPDATE-OR + 9 INSERT-AND. Live-DB unverändert (0 residual triggers post-ROLLBACK) — Live-Apply bleibt /deploy-Ops-Pre-Cond.
