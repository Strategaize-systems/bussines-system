...
| 5 | **NEU** `cockpit/src/app/api/emails/[id]/body/route.ts` | Same-Origin-GET, lädt `body_html` per **User-Client (RLS-scoped)** an **`email_messages.id`** (Plan-QA RPT-676-Korrektur ggü. „emails.id": reale Inbound-Viewer-Quelle — `InboxEmail extends EmailMessage`, `getInboxEmail*` selektieren `email_messages`; `.from('emails')` ist separate Entity), `sanitizeEmailHtml(body,{blockRemoteImages:false})`, Response mit **route-scoped** `Content-Security-Policy: default-src 'none'; img-src https: data: blob: cid:; style-src 'unsafe-inline'; font-src 'self'`. Globale CSP unberührt. Browser lädt Bilder direkt (kein Proxy). | DEC-306 / RPT-676 |
